...
export const SmartCard: React.FC<SmartCardProps> = ({
  ...
  subtitle,
  status,
  statusColor = 'bg-[#2a2a2a] text-slate-200',
  avatarInitials,
  progress,
  deadline,
  state = 'medium',
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl mb-3">
        ...
        <div 
            ...
            className={`relative bg-[#222] p-4 rounded-2xl border border-[#333] shadow-sm cursor-pointer ${isSwiping ? 'transition-none' : 'transition-transform duration-300'} hover:border-indigo-300 hover:shadow-md h-full flex flex-col`}
        >
            <div className="flex justify-between items-start gap-4 mb-2">
                <div className="flex-1 min-w-0">
                    <h4 className={`font-bold text-slate-100 ${state === 'compact' ? 'truncate text-sm' : 'text-base leading-tight line-clamp-2'}`}>
                        {title}
                    </h4>
                    {subtitle && (
