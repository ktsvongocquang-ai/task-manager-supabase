        ...
        {data.roles && data.roles.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {data.roles.map((r: string, i: number) => (
              <span key={i} className="px-2.5 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-md uppercase tracking-wider">
                Dành cho: {r}
              </span>
            ))}
          </div>
          ...
          <div key={group.id} className="relative">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex flex-col items-center">
                ...
                <h4 className="text-lg font-bold text-gray-800">{group.title}</h4>
              </div>
              <div className="h-px bg-gray-200 flex-1" />
            </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
      ...
      <div className="bg-gray-50 border-b border-gray-200 px-5 py-4">
        <h5 className="text-base font-bold text-gray-900 flex items-start gap-2">
        ...
        {principle.rows && principle.rows.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {principle.rows.map((row: any, idx: number) => {
              let colors = "bg-gray-50 border-gray-200 text-gray-800";
        ...
        {principle.specs && principle.specs.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
            {principle.specs.map((spec: string, idx: number) => (
              <span key={idx} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-100 text-gray-600 rounded-md text-xs font-medium border border-gray-200">
...
function MindsetChecklist({ items }: { items: any[] }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
      {items.map((item, idx) => (
        <div key={idx} className="flex items-start gap-3 p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
    ...
    <div className="space-y-4">
      {items.map((item, idx) => (
        <div key={idx} className="bg-red-50 border border-red-100 rounded-xl p-5 relative overflow-hidden">
          ...
          <div className="bg-white/60 p-3 rounded-lg border border-red-200/50">
            <p className="text-xs font-bold text-green-700 uppercase mb-1">Cách phòng tránh:</p>
            <p className="text-sm text-gray-800 font-medium">{item.prevention}</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {items.map((item, idx) => (
        <div key={idx} className="bg-white border border-gray-200 rounded-xl p-5 hover:border-purple-300 transition-colors shadow-sm">
          ...
          <p className="text-sm font-bold text-gray-900 mb-3 italic">
            "{item.question}"
          </p>
          <div className="pt-3 border-t border-gray-100">
            <p className="text-xs text-gray-500 mb-1">Mục đích khai thác:</p>
