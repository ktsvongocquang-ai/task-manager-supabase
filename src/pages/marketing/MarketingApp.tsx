  ...
  return (
    <div className="relative w-full text-left group" ref={dropdownRef} onClick={e => e.stopPropagation()}>
      <div 
        ...
        className="w-full cursor-pointer min-h-[32px] flex items-center justify-between rounded hover:bg-[#1c1c1c] transition-colors px-1"
      >
        <div className="flex-1 truncate">
          {value ? (renderValue ? renderValue(value) : <span className="text-[13px] text-slate-200">{value}</span>) : <span className="text-slate-400 text-xs">- {placeholder} -</span>}
        </div>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 min-w-[160px] max-h-[250px] overflow-y-auto bg-[#222] rounded-lg shadow-xl border border-[#333] py-1" style={{ top: '100%', left: 0 }}>
          {options.map((opt) => (
            <div 
              key={opt}
              onClick={() => { onChange(opt); setIsOpen(false); }}
              className="px-2 py-1.5 hover:bg-[#1c1c1c] cursor-pointer flex items-center"
            >
              {renderOption ? renderOption(opt) : <span className="text-[13px] text-slate-200 px-1 py-0.5">{opt}</span>}
            </div>
          ))}
        </div>
      )}
...
const getProjectStatusColor = (status: string) => {
  if (!status) return 'bg-[#2a2a2a] text-slate-200 border-[#333]';
...
const getProjectEffectColor = (ef: string) => {
  if (!ef) return 'bg-[#2a2a2a] text-slate-200';
  const trimEf = ef.trim();
  if (trimEf === 'Minimal') return 'bg-[#2a2a2a] text-slate-200';
...
const STATUS_MAP: Record<string, { col: string, name: string, color: string, textColor: string }> = {
  ...
  SCHEDULED: { col: 'COL_DONE', name: 'Lên lịch đăng', color: 'bg-[#2a2a2a] text-slate-100 border-[#333]', textColor: 'text-gray-600' },
  ...
  return (
    <div className="w-full mb-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        <div className="flex items-center bg-[#222] rounded-lg p-1 shadow-sm border border-[#333]">
          {['Tất cả', 'Theo Tuần', 'Theo Tháng', 'Theo Quý', 'Theo Năm'].map(t => (
            <button 
              key={t}
              onClick={() => setTimeFilter(t)}
              className={`px-4 py-1.5 text-sm ${timeFilter === t ? 'font-bold bg-[#222] text-slate-100 rounded-md shadow-sm' : 'font-medium text-slate-400 hover:text-slate-200'}`}>
              {t}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-3">
          <div className="w-[180px] bg-[#222] rounded-lg shadow-sm border border-[#333] px-2 py-0.5 relative z-20">
             <InlineDropdown
               value={statusFilter}
               ...
               renderOption={(val) => {
                 ...
                 return (
                   <div className="flex items-center gap-2">
                     ...
                     <span className={`text-[13px] font-bold ${info?.textColor || 'text-slate-200'}`}>{info?.name || val}</span>
                   </div>
                 );
               }}
             />
          </div>
          <div className="w-[180px] bg-[#222] rounded-lg shadow-sm border border-[#333] px-2 py-0.5 relative z-10">
             <InlineDropdown
               value={formatFilter}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Bar Chart Section */}
        <div className="bg-[#222] p-6 rounded-2xl shadow-sm border border-[#333] flex flex-col">
          <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center mb-6">SỐ LƯỢNG CONTENT MỖI NỀN TẢNG</h3>
        ...
        {/* Metrics Section */}
        <div className="bg-[#222] p-6 rounded-2xl shadow-sm border border-[#333] flex flex-col justify-between">
          <div className="flex justify-between items-start mb-6 w-full">
             <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">HIỆU QUẢ CONTENT</h3>
             <div className="text-right">
               <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">TỔNG BÀI VIẾT</p>
               <p className="text-2xl font-black text-slate-100 leading-none">{currentData.posts}</p>
             </div>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
            {/* Views */}
            <div className="border border-indigo-50/60 bg-[#222] rounded-2xl p-4 sm:p-5 flex items-center gap-4 sm:gap-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 mt-0.5">SỐ LƯỢT XEM</p>
                <p className="text-2xl font-black text-slate-50 tracking-tight">{currentData.views.toLocaleString('vi-VN')}</p>
              </div>
            </div>
            ...
            <div className="border border-indigo-50/60 bg-[#222] rounded-2xl p-4 sm:p-5 flex items-center gap-4 sm:gap-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 mt-0.5">SỐ LƯỢT TƯƠNG TÁC</p>
                <p className="text-2xl font-black text-slate-50 tracking-tight">{currentData.interactions.toLocaleString('vi-VN')}</p>
              </div>
            </div>
            ...
            <div className="border border-indigo-50/60 bg-[#222] rounded-2xl p-4 sm:p-5 flex items-center gap-4 sm:gap-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 mt-0.5">SỐ LƯỢT CHIA SẺ</p>
                <p className="text-2xl font-black text-slate-50 tracking-tight">{currentData.shares.toLocaleString('vi-VN')}</p>
              </div>
            </div>
            ...
            <div className="border border-indigo-50/60 bg-[#222] rounded-2xl p-4 sm:p-5 flex items-center gap-4 sm:gap-5 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1 mt-0.5">SỐ LƯỢT LƯU LẠI</p>
                <p className="text-2xl font-black text-slate-50 tracking-tight">{currentData.saves.toLocaleString('vi-VN')}</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`flex-1 flex flex-col gap-4 w-full min-h-0 min-w-0 px-1 md:px-4 custom-scrollbar ${view === 'TIMELINE' ? 'overflow-hidden' : 'overflow-y-scroll'}`}>
      ...
      <div className="flex flex-col justify-between items-start md:items-center gap-4 shrink-0 px-1 md:px-0 pt-2 pb-1">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full min-h-[64px] min-w-0">
          <div className="flex-1 min-w-0 pr-4">
            <h1 className="text-xl font-bold text-slate-100 hidden md:block">Marketing Workflow</h1>
            <p className="text-sm text-slate-400 mt-1 truncate">Quy trình phối hợp DQH & Team Coach Hiếu</p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-auto mt-4 md:mt-0 justify-start md:justify-end shrink-0">
            ...
            <div className="md:hidden w-full relative shrink-0">
              <select 
                ...
                className="w-full appearance-none bg-[#222] border border-[#333] text-slate-100 text-sm rounded-xl px-4 py-3 font-semibold shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all pr-10"
              >
            ...
            <div className="hidden md:flex flex-wrap items-center bg-[#2a2a2a] p-1 rounded-lg">
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'DASHBOARD' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('DASHBOARD')}
              >
                Tổng quan
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'WORKFLOW' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('WORKFLOW')}
              >
                Tài khoản
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'KANBAN' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('KANBAN')}
              >
                Công việc
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'LIST' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('LIST')}
              >
                Tổng hợp
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'CALENDAR' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('CALENDAR')}
              >
                Lịch đăng
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'TIMELINE' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                onClick={() => handleViewChange('TIMELINE')}
              >
                Tiến độ
              </button>
              <button 
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${view === 'ARCHIVE' ? 'bg-[#222] text-slate-50 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
            ...
            {canEdit && (
              <button 
                ...
                className="bg-[#222] border border-indigo-200 text-indigo-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-50 transition-colors flex items-center gap-2 shadow-sm shrink-0 whitespace-nowrap"
              >
          ...
          <div className="flex flex-col gap-2 min-w-0 w-full mb-2">
            ...
            <div className="hidden md:flex justify-end w-full min-w-0">
              <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto hide-scrollbar pb-1 sm:pb-0">
                <button onClick={() => setShowKanbanFilters(!showKanbanFilters)} className={`flex items-center gap-2 px-3 py-1.5 border border-[#333] rounded-md text-xs font-medium transition-colors shrink-0 whitespace-nowrap ${showKanbanFilters ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-[#222] text-slate-200 hover:bg-[#1c1c1c]'}`}>
                  <Filter className="w-3.5 h-3.5" /> Lọc
                </button>
                <button onClick={() => setView('CALENDAR')} className="flex items-center gap-2 px-3 py-1.5 bg-[#222] border border-[#333] rounded-md text-xs font-medium text-slate-200 hover:bg-[#1c1c1c] transition-colors shrink-0 whitespace-nowrap">
            ...
            {showKanbanFilters && (
              <div className="hidden md:flex flex-wrap items-center gap-3 pt-2 text-sm min-w-0">
                <div className="inline-flex overflow-x-auto hide-scrollbar bg-[#222] border border-[#333] p-1 rounded-lg shadow-sm max-w-full">
                  {['Tất cả', 'Theo Tuần', 'Theo Tháng', 'Theo Quý', 'Theo Năm'].map(filter => (
                    <button
                      key={filter}
                      onClick={() => setListTimeFilter(filter)}
                      className={`px-3 py-1 rounded-md text-xs font-medium transition-colors shrink-0 whitespace-nowrap ${listTimeFilter === filter ? 'bg-indigo-50 text-indigo-700 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                    >
                      {filter}
                    </button>
                  ))}
                </div>
                <select value={formatFilter} onChange={(e) => setFormatFilter(e.target.value)} className="px-3 py-1.5 border border-[#333] rounded-lg text-xs bg-[#222] text-slate-200 shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 shrink-0">
                ...
                <select value={projectFilter} onChange={(e) => setProjectFilter(e.target.value)} className="px-3 py-1.5 border border-[#333] rounded-lg text-xs bg-[#222] text-slate-200 shadow-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 shrink-0 max-w-[150px] truncate">
                ...
                {(listTimeFilter !== 'Tất cả' || formatFilter !== 'Tất cả' || projectFilter !== 'Tất cả') && (
                  <button onClick={() => { setListTimeFilter('Tất cả'); setFormatFilter('Tất cả'); setProjectFilter('Tất cả'); }} className="text-xs text-gray-400 hover:text-slate-200 underline shrink-0 whitespace-nowrap">
                    Xóa lộc
                  </button>
                )}
              </div>
      ...
      {view === 'KANBAN' ? (
        <>
           ...
           <div className="hidden md:flex flex-1 gap-4 overflow-x-auto pb-4 hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0 md:min-h-[500px]">
              {COLUMNS.map(column => {
                ...
                return (
                  <div key={column.id} className={`w-[90vw] sm:w-[300px] md:w-[320px] h-full bg-[#222] rounded-2xl border ${column.border} flex flex-col shrink-0 relative max-h-full`}>
                    <div className={`p-4 border-b flex items-center justify-between rounded-t-2xl shadow-sm shrink-0 ${column.color}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="bg-[#222] text-current px-2.5 py-0.5 rounded-full text-xs font-bold shadow-sm">
                          {activeVideos.length}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button 
                            ...
                            className="bg-[#222]/80 hover:bg-[#222] text-current p-1 rounded transition-colors shadow-sm"
                            title="Thêm Task mới"
                        >
                            <Plus size={16} />
                        </button>
                        {archivedVideos.length > 0 && (
                          <button
                            ...
                            className="flex items-center gap-1 text-red-600 bg-[#222] border border-red-500 hover:bg-red-50 px-2 py-0.5 rounded-md transition-colors shadow-sm"
                      ...
                      <div className="absolute top-16 left-2 right-2 z-50 bg-yellow-50/95 backdrop-blur-sm border border-yellow-200 shadow-xl rounded-xl p-3 max-h-[400px] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                          {archivedVideos.map(video => (
                            <div key={video.id} className="bg-[#222] border border-yellow-200 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
                              <h4 className="text-xs font-bold text-slate-100 mb-1 line-clamp-2">{video.title}</h4>
                              <p className="text-[10px] text-slate-400 mb-2 truncate flex items-center gap-1"><LayoutTemplate className="w-3 h-3"/> {video.project}</p>
                      ...
                      {activeVideos.map(video => {
                        ...
                        return (
                            <div 
                              ...
                              className="bg-[#222] p-3 rounded-xl shadow-sm border border-[#333] hover:border-indigo-300 hover:shadow-md transition-all cursor-grab active:cursor-grabbing flex flex-col gap-2 relative group"
                            >
                              {/* Header Badges and Actions */}
                              <div className="flex justify-between items-start mb-1">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border min-h-[22px] flex items-center ${statusDef?.color || 'bg-[#2a2a2a] text-slate-100'}`}>
                                  ...
                                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border min-h-[22px] flex items-center ${task.platform && task.platform !== 'Khác' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-[#2a2a2a] text-gray-600 border-[#333]'}`}>
                              ...
                              <div className="flex justify-between items-center gap-2">
                                <h3 className={`font-bold text-slate-50 text-[14px] leading-tight flex-1 ${!expandedCards[task.id] ? 'line-clamp-2' : ''}`}>
                                  {task.title}
                                </h3>
                                <div className="flex items-center gap-1 bg-[#222] text-slate-400 font-bold px-1.5 py-0.5 rounded border border-[#333] text-[11px] shrink-0">
                              ...
                              {expandedCards[task.id] && (task.project || task.notes || task.result_links || task.description) && (
                                <div className="mt-1 flex flex-col gap-2 pt-3 border-t border-[#333] animate-in slide-in-from-top-2 duration-200 text-[13px]">
                                  {task.project && (
                                    <div className="grid grid-cols-[100px_1fr] gap-2 items-start">
                                      <span className="text-slate-400 font-medium">Dự án:</span>
                                      <span className="text-slate-50">{task.project}</span>
                                    </div>
                                  )}
                                  {task.notes && (
                                    <div className="grid grid-cols-[100px_1fr] gap-2 items-start">
                                      <span className="text-slate-400 font-medium">Hook chọn:</span>
                                      <span className="text-slate-50">{task.notes}</span>
                                    </div>
                                  )}
                                  {task.result_links && (
                                    <div className="grid grid-cols-[100px_1fr] gap-2 items-start">
                                      <span className="text-slate-400 font-medium">Vấn đề:</span>
                                      <span className="text-slate-50">{task.result_links}</span>
                                    </div>
                                  )}
                                  {task.description && (
                                    <div className="grid grid-cols-[100px_1fr] gap-2 items-start">
                                      <span className="text-slate-400 font-medium">Giải pháp:</span>
                                      <span className="text-slate-50 line-clamp-3">{task.description}</span>
                                    </div>
                                  )}
                                </div>
                              )}
                                  ...
                                  {task.status === 'IDEA' && (
                                    <div className="flex gap-2 w-full mt-2">
                                      ...
                                      <button 
                                        ...
                                        className="flex-1 py-1.5 bg-[#222] text-red-600 border border-red-200 hover:bg-red-50 rounded-lg text-[12px] font-bold transition-colors"
                                      >
                                        Từ chối
                                      </button>
                                      <button 
                                        onClick={(e) => { e.stopPropagation(); updateTask(task.id, { isarchived: true }); }}
                                        className="flex-1 py-1.5 bg-[#222] text-gray-600 border border-[#333] hover:bg-[#1c1c1c] rounded-lg text-[12px] font-bold transition-colors"
                                      >
                                        Để sau
                                      </button>
                                    </div>
            ...
            <div className="md:hidden pb-24 border-y border-[#333] mt-2 bg-[#222]">
                {COLUMNS.map((column, index) => {
                    ...
                    return (
                        <div key={column.id} className={`bg-[#222] overflow-hidden flex flex-col ${!isLast ? 'border-b border-[#333]' : ''}`}>
                            {/* Accordion Header */}
                            <div 
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <h3 className="font-bold text-slate-200">{column.name}</h3>
                                    {activeVideos.length > 0 && (
                                        <span className="bg-[#2a2a2a] text-slate-600 px-2 py-0.5 rounded-full text-xs font-bold">{activeVideos.length}</span>
                                    )}
                                </div>
                                        ...
                                        activeVideos.map(video => {
                                            ...
                                            return (
                                              <div 
                                                ...
                                                className="bg-[#222] p-4 rounded-2xl shadow-sm border border-[#333] active:border-indigo-300 transition-all cursor-pointer flex flex-col gap-3"
                                              >
                                                <div className="flex justify-between items-start mb-1">
                                                  <div className="flex items-center gap-1.5 flex-wrap">
                                                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border min-h-[22px] flex items-center ${statusDef?.color || 'bg-[#2a2a2a] text-slate-100'}`}>
                                                ...
                                                {isCardExpanded && (
                                                  <div className="mt-2 space-y-2">
                                                    ...
                                                    <div className="flex gap-1.5">
                                                      <span className="text-[10px] bg-[#1c1c1c] text-slate-400 px-2 py-1 rounded-md flex items-center gap-1 border border-[#333]">
                                                  ...
                                                  <div className="flex items-center justify-between pt-3 border-t border-[#333] mt-2">
                                                      <div className="flex items-center gap-2">
                                                        <span className="text-[11px] font-medium text-slate-400">Thực hiện</span>
        ...
        <div className="flex-1 overflow-auto bg-[#222] rounded-xl shadow-sm border border-[#333] mx-1 md:mx-0 flex flex-col min-w-0">
           <div className="px-4 py-4 md:px-6 md:py-5 border-b border-[#333] flex justify-between items-center bg-[#222] sticky top-0 z-20 rounded-t-xl min-w-0">
             <div className="flex items-center gap-2">
               <ListTodo className="w-5 h-5 text-indigo-600" />
               <h2 className="text-lg font-bold text-slate-50 uppercase tracking-widest">TỔNG HỢP DỰ ÁN</h2>
             </div>
             <div className="flex items-center gap-3">
                <div className="flex bg-[#2a2a2a]/50 p-1 rounded-xl border border-[#333] shadow-inner gap-1">
                    <div className="min-w-[140px] bg-[#222] rounded-lg shadow-sm border border-[#333] px-2 py-0.5 relative z-20">
                         <InlineDropdown
                    ...
                    <div className="min-w-[140px] bg-[#222] rounded-lg shadow-sm border border-[#333] px-1 py-0.5 relative z-20">
                         <InlineDropdown
                           ...
                           renderOption={(val) => {
                             ...
                             return (
                               <div className="flex items-center gap-2">
                                 ...
                                 <span className={`text-[12px] font-bold ${info?.textColor || 'text-slate-200'}`}>{info?.name || val}</span>
                               </div>
                             );
                           }}
                         />
           ...
           <div className="flex-1 overflow-x-auto overflow-y-auto p-0 w-full hidden md:block">
             <table className="w-full text-[13px] text-left border-collapse min-w-[1000px] border border-[#333]">
               <thead className="bg-[#f9fafb] text-slate-400 sticky top-0 z-10">
                 <tr>
                   <th className="px-3 py-2 border border-[#333] font-normal w-[40px] text-center"></th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[200px]">Dự án</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[120px]">Loại</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[150px]">Trạng thái</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[140px]">Phong cách</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[280px]">Tiến độ & Nhật ký</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[150px]">Điểm nhấn</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[180px]">Nội dung khai thác</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[150px]">Brief</th>
                   <th className="px-3 py-2 border border-[#333] font-normal min-w-[150px]">Ghi chú</th>
                 </tr>
                </thead>
                <tbody className="divide-y divide-slate-200">
                  {(() => {
                    ...
                    if (filteredProjects.length === 0) {
                        return (
                            <tr>
                                <td colSpan={10} className="px-4 py-12 text-center text-slate-400 bg-[#222]">
                    ...
                    return filteredProjects.map((proj: any) => {
                      ...
                      return (
                         <React.Fragment key={proj.id}>
                           <tr className="bg-[#222] hover:bg-[#1c1c1c] transition-colors cursor-pointer group" onClick={(e) => toggleProjectRow(proj.id, e)} onDoubleClick={() => { setEditingProject(proj); setIsProjectModalOpen(true); }}>
                             <td className="px-3 py-2 border border-[#333] text-center text-slate-400 group-hover:text-slate-600 font-medium cursor-pointer">
                               {expandedProjects.has(proj.id) || taskStatusFilter !== 'Tất cả' ? (
                                  <svg className="w-4 h-4 mx-auto text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                               ) : (
                             ...
                             <td className="px-3 py-2 border border-[#333] text-slate-50 font-semibold">{proj.name}</td>
                             <td className="px-3 py-2 border border-[#333]">
                               <InlineDropdown
                             ...
                             <td className="px-3 py-2 border border-[#333]" onClick={e => e.stopPropagation()}>
                               <InlineDropdown
                               ...
                               <div className="flex flex-col gap-2 relative">
                                  <div className="flex items-center gap-2">
                                       {phaseStr ? (
                                           ...
                                           <span className="bg-[#1c1c1c] text-slate-400 border border-[#333] text-[10px] font-medium px-2 py-0.5 rounded">Chưa setup</span>
                                       )}
                                       {upcomingMilestone && (
                                  ...
                                  {latestLog ? (
                                      <div className="text-[11px] text-gray-600 bg-[#1c1c1c] p-2 rounded-lg border border-[#333]/50">
                             ...
                             <td className="px-3 py-2 border border-[#333]" title={proj.effect_description || ''}>
                               <div className="text-[12px] font-medium text-slate-200 line-clamp-2">{proj.effect_description || '-'}</div>
                             </td>
                             <td className="px-3 py-2 border border-[#333]" title={proj.customer_problem || ''}>
                               <div className="text-[12px] font-medium text-slate-200 line-clamp-2">{proj.customer_problem || '-'}</div>
                             </td>
                             <td className="px-3 py-2 border border-[#333]">
                               {proj.content_link ? (
                             ...
                             <td className="px-3 py-2 border border-[#333]" title={proj.other_info || ''}>
                               <div className="text-[12px] font-medium text-slate-200 line-clamp-2">{proj.other_info || '-'}</div>
                             </td>
                           </tr>
                           {(expandedProjects.has(proj.id) || taskStatusFilter !== 'Tất cả') && (
                             <tr>
                               <td colSpan={10} className="p-0">
                                 <div className="bg-[#1c1c1c] p-4 border-t border-[#333]">
                                   <div className="flex items-center gap-3 mb-3">
                                     <button 
                                       onClick={() => { setEditingTask({ project_id: proj.id, status: 'IDEA' } as any); setIsTaskModalOpen(true); }}
                                       className="bg-[#222] text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors flex items-center gap-1 shadow-sm"
                                       title="Thêm Idea"
                                     >
                                       <Plus className="w-3.5 h-3.5" /> Thêm Idea
                                     </button>
                                     <h4 className="text-sm font-bold text-slate-200">Tasks cho dự án "{proj.name}"</h4>
                                   </div>
                                   ...
                                   <Droppable droppableId={`proj-${proj.id}`}>
                                     {(provided) => (
                                       <table className="w-full text-[12px] text-left border-collapse border border-[#333]" ref={provided.innerRef} {...provided.droppableProps}>
                                         <thead className="bg-[#2a2a2a] text-slate-400">
                                           <tr>
                                             <th className="px-3 py-2 border border-[#333] font-normal w-[30px] text-center"></th>
                                             <th className="px-3 py-2 border border-[#333] font-bold w-[40px] text-center">STT</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[150px]">Tiêu đề</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[100px]">Nền tảng</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[100px]">Link</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[130px]">Ngày đăng</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[120px]">Trạng thái</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal min-w-[100px]">Người phụ trách</th>
                                             <th className="px-3 py-2 border border-[#333] font-normal w-[80px] text-center"></th>
                                           </tr>
                                         </thead>
                                         <tbody className="divide-y divide-slate-200">
                                       ...
                                       {videos.filter(v => v.project_id === proj.id && (taskStatusFilter === 'Tất cả' || v.status === taskStatusFilter)).length === 0 ? (
                                         <tr>
                                           <td colSpan={10} className="px-4 py-6 text-center text-slate-400 bg-[#222]">
                                            ...
                                            <Draggable key={video.id} draggableId={video.id} index={index}>
                                              {(provided, snapshot) => (
                                               <tr
                                                 ...
                                                 className={`bg-[#222] hover:bg-[#1c1c1c] transition-colors ${snapshot.isDragging ? 'shadow-lg border border-indigo-300' : ''}`}
                                               >
                                                 <td className="px-2 py-2 border border-[#333] text-center text-slate-300 hover:text-indigo-500">
                                                   <div {...provided.dragHandleProps} className="flex justify-center cursor-grab active:cursor-grabbing p-1">
                                                     <GripVertical size={16} />
                                                   </div>
                                                 </td>
                                                 <td className="px-3 py-2 border border-[#333] text-center font-bold text-slate-400">
                                                   {String(index + 1).padStart(2, '0')}
                                                 </td>
                                                 <td 
                                                   className="px-3 py-2 border border-[#333] font-medium text-indigo-600 hover:text-indigo-800 cursor-pointer hover:underline transition-colors"
                                                   onClick={() => { setEditingTask(video); setIsTaskModalOpen(true); }}
                                                 >
                                                   {video.title}
                                                 </td>
                                                 <td className="px-3 py-2 border border-[#333]">{video.platform}</td>
                                                 <td className="px-3 py-2 border border-[#333]">
                                                   {video.link && (
                                                 ...
                                                 <td className="px-3 py-2 border border-[#333]">
                                                   <div className="flex flex-col gap-1">
                                                     <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-200" title="Ngày đăng">
                                                 ...
                                                 <td className="px-3 py-2 border border-[#333]">{video.assignee.split(',')[0]}</td>
                                                 <td className="px-3 py-2 border border-[#333] text-center">
           ...
           {/* Mobile List View */}
           <div className="md:hidden flex-1 overflow-y-auto p-3 flex flex-col gap-3 bg-[#1c1c1c]/50">
             {(() => {
               const filteredProjects = projects.filter(p => {
                   if (projectStatusFilter !== 'Tất cả' && p.update_status !== projectStatusFilter) return false;
                   if (taskStatusFilter !== 'Tất cả') {
               ...
               if (filteredProjects.length === 0) {
                   return <div className="p-8 text-center text-slate-400 bg-[#222] rounded-xl border border-[#333] shadow-sm">Không tìm thấy dự án phù hợp</div>;
               }
               ...
               return filteredProjects.map(proj => {
                 ...
                 return (
                   <div key={proj.id} className="bg-[#222] rounded-xl shadow-sm border border-[#333] overflow-hidden flex flex-col">
                     {/* Project Header */}
                     <div 
                       className="p-4 flex items-center justify-between cursor-pointer active:bg-[#1c1c1c] transition-colors"
                       onClick={(e) => toggleProjectRow(proj.id, e)}
                     >
                       <div className="flex-1 pr-4">
                         <h4 className="font-bold text-slate-50 text-[15px]">{proj.name}</h4>
                         <div className="flex flex-wrap gap-2 mt-2">
                           ...
                           <span className="bg-[#2a2a2a] text-slate-600 px-2 py-0.5 rounded-sm text-[10px] font-medium border border-[#333]">
                             {projTasks.length} tasks
                           </span>
                         </div>
                       </div>
                       <div className="w-8 h-8 rounded-full bg-[#1c1c1c] flex items-center justify-center text-slate-400 shrink-0 border border-[#333] transition-transform">
                     ...
                     {/* Expanded Tasks Area */}
                     {isExpanded && (
                       <div className="bg-[#1c1c1c]/80 border-t border-[#333] p-3 flex flex-col gap-3">
                         <div className="flex items-center gap-2 px-1 mb-1">
                           <button 
                             onClick={() => { setEditingTask({ project_id: proj.id, status: 'IDEA' } as any); setIsTaskModalOpen(true); }}
                             className="bg-[#222] text-indigo-600 border border-indigo-200 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg text-[11px] font-bold transition-colors flex items-center gap-1 shadow-sm"
                           >
                             <Plus className="w-3.5 h-3.5" /> Thêm Idea
                           </button>
                           <span className="text-[11px] font-bold text-slate-400 tracking-wide uppercase">Ideas & Tasks</span>
                         </div>

                         {projTasks.length === 0 ? (
                           <div className="text-center text-slate-400 text-[12px] py-5 bg-[#222] rounded-xl border border-[#333] border-dashed">
                           ...
                           projTasks.map((task: any, index: number) => {
                             ...
                             return (
                               <div key={task.id} className="bg-[#222] p-3.5 rounded-xl shadow-sm border border-[#333] relative hover:border-indigo-300 transition-colors cursor-pointer" onClick={() => { setEditingTask(task); setIsTaskModalOpen(true); }}>
                                 <div className="flex justify-between items-start gap-3 mb-2">
                                   <h5 className="font-bold text-slate-100 text-[13px] leading-snug flex-1">{task.title}</h5>
                                   <div className="text-[10px] font-bold text-slate-400 bg-[#1c1c1c] px-1.5 py-0.5 rounded-md border border-[#333] mt-0.5 shrink-0">
                                     #{String(index + 1).padStart(2, '0')}
                                   </div>
                                 </div>
                                 ...
                                 <div className="flex flex-wrap gap-1.5 mb-3">
                                   ...
                                   {task.platform && task.platform !== 'Khác' && (
                                     <span className="bg-[#1c1c1c] text-slate-400 border border-[#333] text-[10px] font-medium px-2 py-0.5 rounded-full flex items-center gap-1">
                                       {task.platform}
                                     </span>
                                   )}
                                 </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#222]">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-lg font-bold text-slate-50 min-w-[120px] text-center">
            ...
            <div className="border border-[#333] rounded-xl overflow-hidden">
              <div className="grid grid-cols-7 bg-[#1c1c1c] border-b border-[#333]">
                {['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'CN'].map(day => (
                  <div key={day} className="py-3 text-center text-sm font-bold text-slate-200">
                    {day}
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-7 bg-[#222]">
                {(() => {
                  ...
                  while (day <= endDate) {
                    ...
                    days.push(
                      <div 
                        key={day.toString()} 
                        className={`min-h-[120px] p-2 border-r border-b border-[#333] ${!isCurrentMonth ? 'bg-[#1c1c1c] text-gray-400' : 'bg-[#222]'}`}
                      >
        ...
        <div 
          className="flex-1 min-h-0 min-w-0 flex flex-col p-2 md:p-4 bg-[#1c1c1c] relative overflow-hidden transition-all duration-300 rounded-2xl"
        >
          <ProjectGanttBoard />
        </div>
      ) : view === 'DASHBOARD' ? (
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-[#1c1c1c] relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-[#1c1c1c] relative">
          <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
            ...
            {profile && (
              <>
                <div className="bg-[#222] rounded-3xl shadow-sm border border-[#333]/60 overflow-hidden">
                    ...
                    <div className="px-6 pb-6 relative">
                        <div className="flex flex-col sm:flex-row items-center sm:items-end -mt-12 sm:-mt-16 gap-4 sm:gap-6">
                            <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-3xl bg-pink-500 border-4 border-white shadow-xl flex flex-col justify-center items-center`}>
                                ...
                                <div className="absolute -bottom-3 bg-[#222] px-3 py-1 rounded-full shadow-lg border border-[#333] flex items-center gap-1.5">
                            ...
                            <div className="text-center sm:text-left pt-2 flex-1">
                                <h1 className="text-xl sm:text-2xl font-black text-slate-50 tracking-tight mb-1">{profile?.full_name || 'Marketing Admin'}</h1>
                                <p className="text-slate-400 text-sm font-medium flex items-center justify-center sm:justify-start gap-1.5">
                                    <Mail size={14} /> 
                    ...
                    <div className="prose prose-slate prose-sm max-w-none">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="p-4 rounded-xl border bg-indigo-50/50 border-indigo-100">
                                ...
                                <p className="text-xs text-slate-400">Sử dụng Kanban Marketing, đăng bài, lên lịch.</p>
                            </div>
                            <div className="p-4 rounded-xl border bg-amber-50/50 border-amber-100">
                                ...
                                <p className="text-xs text-slate-400">Chỉ xem tiến độ để viết bài PR truyền thông.</p>
                            </div>
                            <div className="p-4 rounded-xl border bg-[#1c1c1c] border-[#333] opacity-80">
                                <h4 className="text-sm font-bold mb-1 text-slate-400">Dự án chung</h4>
                                <p className="text-xs text-slate-400">Bị giới hạn, không truy cập bảng tổng công ty.</p>
                            </div>
                            <div className="p-4 rounded-xl border bg-[#1c1c1c] border-[#333] opacity-80">
                                <h4 className="text-sm font-bold mb-1 text-slate-400">Khách hàng (CRM)</h4>
                                <p className="text-xs text-slate-400">Chỉ Sale và Admin được phép truy cập.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="text-center pt-4 mb-8">
              <h2 className="text-2xl md:text-3xl font-black text-slate-50 uppercase tracking-widest mb-2">QUY CHUẨN LÀM VIỆC</h2>
              <p className="text-slate-400">Quy trình phối hợp DQH & Team Coach Hiếu</p>
            </div>

            {/* Phần 1: Tổng quan */}
            <section className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden">
              ...
              <div className="p-6">
                <table className="w-full text-sm text-left mb-6 border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Kênh trao đổi chính</td>
                      <td className="px-4 py-3 border border-[#333]">Zalo group</td>
                    </tr>
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Đầu mối DQH</td>
                      <td className="px-4 py-3 border border-[#333]">Anh Minh (Co-founder)</td>
                    </tr>
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Đầu mối Coach</td>
                      <td className="px-4 py-3 border border-[#333]">Anh Hiếu + Team Vy Trúc</td>
                    </tr>
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Output mục tiêu</td>
                      <td className="px-4 py-3 border border-[#333]">3 video/tuần (12 video/tháng)</td>
                    </tr>
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Sync cố định</td>
                      <td className="px-4 py-3 border border-[#333]">1 lần/tuần (Thứ 3)</td>
                    </tr>
                  </tbody>
                </table>
            ...
            <section className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden">
              ...
              <div className="p-6">
                <table className="w-full text-sm text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 2</td>
                      <td className="px-4 py-3 border border-[#333]">Nhận update công trình</td>
                      <td className="px-4 py-3 border border-[#333]">Gửi update công trình</td>
                    </tr>
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 3</td>
                      <td className="px-4 py-3 border border-[#333]">Gửi outline + hook</td>
                      <td className="px-4 py-3 border border-[#333]">-</td>
                    </tr>
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 4</td>
                      <td className="px-4 py-3 border border-[#333]">-</td>
                      <td className="px-4 py-3 border border-[#333]">Điền chuyên môn + duyệt</td>
                    </tr>
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 5-6</td>
                      <td className="px-4 py-3 border border-[#333]">-</td>
                      <td className="px-4 py-3 border border-[#333]">Quay footage</td>
                    </tr>
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 6 tối</td>
                      <td className="px-4 py-3 border border-[#333]">-</td>
                      <td className="px-4 py-3 border border-[#333]">Upload source</td>
                    </tr>
                    <tr className="bg-[#222]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 7-CN</td>
                      <td className="px-4 py-3 border border-[#333]">Edit + QC</td>
                      <td className="px-4 py-3 border border-[#333]">-</td>
                    </tr>
                    <tr className="bg-[#1c1c1c]">
                      <td className="px-4 py-3 border border-[#333] font-medium">Thứ 2 (tuần sau)</td>
                      <td className="px-4 py-3 border border-[#333]">Gửi video final</td>
                      <td className="px-4 py-3 border border-[#333]">Duyệt + Đăng</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <section className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-8">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 mb-1"><strong>Deadline:</strong> Thứ 2 hàng tuần</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Ai làm:</strong> Anh Minh / Bạn media nội bộ</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Output:</strong> Bảng cập nhật công trình theo format:</p>
                  <table className="w-full text-xs text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      <tr className="bg-[#1c1c1c]">
                        <td className="px-3 py-2 border border-[#333]">Infiniti 2PN</td>
                        <td className="px-3 py-2 border border-[#333]">Thi công trần</td>
                        <td className="px-3 py-2 border border-[#333]">Lắp hệ thống chống ồn</td>
                        <td className="px-3 py-2 border border-[#333]">✓</td>
                        <td className="px-3 py-2 border border-[#333]">T4-T5</td>
                      </tr>
                      <tr className="bg-[#222]">
                        <td className="px-3 py-2 border border-[#333]">Landmark 3PN</td>
                        <td className="px-3 py-2 border border-[#333]">Thiết kế</td>
                        <td className="px-3 py-2 border border-[#333]">Vừa chốt layout</td>
                        <td className="px-3 py-2 border border-[#333]">X</td>
                        <td className="px-3 py-2 border border-[#333]">-</td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 mb-1"><strong>Deadline:</strong> Thứ 3</p>
                  <p className="text-sm text-slate-200 mb-1"><strong>Ai làm:</strong> Team Vy Trúc + Anh Hiếu duyệt</p>
                  <p className="text-sm text-slate-200"><strong>Output:</strong> Bảng content tuần bao gồm hook options, angle, shot list</p>
                </div>

                {/* Bước 3 */}
                <div>
                  <h4 className="text-md font-bold text-amber-900 mb-1">BƯỚC 3: DQH điền chuyên môn + duyệt</h4>
                  <p className="text-sm text-slate-200 mb-1"><strong>Deadline:</strong> Thứ 4 trưa</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Ai làm:</strong> Anh Minh / Anh Quang</p>
                  <p className="text-sm text-slate-200 mb-1"><strong>Làm gì:</strong></p>
                  <ul className="list-disc list-inside text-sm text-slate-200 space-y-1 ml-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 mb-1"><strong>Thời gian:</strong> Thứ 5-6</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Ai làm:</strong> Bạn media nội bộ + Anh Minh/Quang</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Shot list chuẩn cho mỗi video:</strong></p>
                  <table className="w-full text-xs text-left border-collapse mb-2">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      <tr className="bg-[#1c1c1c]">
                        <td className="px-3 py-2 border border-[#333]">A</td>
                        <td className="px-3 py-2 border border-[#333]">Hook - cận mặt nói</td>
                        <td className="px-3 py-2 border border-[#333]">5-10s</td>
                        <td className="px-3 py-2 border border-[#333]">Anh Minh</td>
                      </tr>
                      <tr className="bg-[#222]">
                        <td className="px-3 py-2 border border-[#333]">B</td>
                        <td className="px-3 py-2 border border-[#333]">Cảnh rộng công trình</td>
                        <td className="px-3 py-2 border border-[#333]">3-5s</td>
                        <td className="px-3 py-2 border border-[#333]">Media</td>
                      </tr>
                      <tr className="bg-[#1c1c1c]">
                        <td className="px-3 py-2 border border-[#333]">C</td>
                        <td className="px-3 py-2 border border-[#333]">Cận chi tiết (vật liệu, thi công)</td>
                        <td className="px-3 py-2 border border-[#333]">10-15s</td>
                        <td className="px-3 py-2 border border-[#333]">Media</td>
                      </tr>
                      <tr className="bg-[#222]">
                        <td className="px-3 py-2 border border-[#333]">D</td>
                        <td className="px-3 py-2 border border-[#333]">Giải thích tại hiện trường</td>
                        <td className="px-3 py-2 border border-[#333]">20-30s</td>
                        <td className="px-3 py-2 border border-[#333]">Anh Minh/Quang</td>
                      </tr>
                      <tr className="bg-[#1c1c1c]">
                        <td className="px-3 py-2 border border-[#333]">E</td>
                        <td className="px-3 py-2 border border-[#333]">Before/After (nếu có)</td>
                        <td className="px-3 py-2 border border-[#333]">5-10s</td>
                        <td className="px-3 py-2 border border-[#333]">Media</td>
                      </tr>
                    </tbody>
                  </table>
                  <p className="text-sm text-slate-200 italic">Lưu ý: Quay DỌC (9:16) • Quay thừa, đừng thiếu • Raw footage OK</p>
                </div>

                {/* Bước 5 */}
                <div>
                  <h4 className="text-md font-bold text-amber-900 mb-1">BƯỚC 5: DQH upload source</h4>
                  <p className="text-sm text-slate-200 mb-1"><strong>Deadline:</strong> Thứ 6 tối</p>
                  <p className="text-sm text-slate-200 mb-1"><strong>Ai làm:</strong> Bạn media nội bộ</p>
                  <p className="text-sm text-slate-200 mb-1"><strong>Upload lên:</strong> Google Drive folder chung</p>
                  <p className="text-sm text-slate-200"><strong>Cấu trúc folder:</strong> DQH_Content / Tuần_X / Video_TênCôngTrình / các file .mp4</p>
                </div>

                {/* Bước 6 */}
                <div>
                  <h4 className="text-md font-bold text-amber-900 mb-1">BƯỚC 6: Team Hiếu edit + QC</h4>
                  <p className="text-sm text-slate-200 mb-1"><strong>Thời gian:</strong> Thứ 7 - Chủ Nhật</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Ai làm:</strong> Editor + Team Vy Trúc QC + Anh Hiếu duyệt final</p>
                  <p className="text-sm text-slate-200 mb-1"><strong>Checklist QC:</strong></p>
                  <ul className="list-disc list-inside text-sm text-slate-200 space-y-1 ml-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-200 mb-1"><strong>Deadline:</strong> Thứ 2 tuần sau</p>
                  <p className="text-sm text-slate-200 mb-2"><strong>Ai làm:</strong> Anh Minh duyệt, Bạn media đăng</p>
                  <p className="text-sm text-slate-200"><strong>Checklist duyệt:</strong> Thông tin chuyên môn chính xác • Hình ảnh công trình OK • Không có gì nhạy cảm</p>
                </div>
              </div>
            </section>
            ...
            <section className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                <div>
                  <h4 className="font-bold text-slate-50 mb-2">Quy tắc "Không chờ"</h4>
                  <ul className="list-disc list-inside text-sm text-slate-200 space-y-2 ml-2">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#1c1c1c]">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
              <h2 className="text-xl font-bold text-slate-50 flex items-center gap-2 shrink-0">
              ...
              <div className="relative w-full sm:max-w-xs">
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-[#333] rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors bg-[#222] shadow-sm"
                />
            ...
            {videos.filter(v => (v.status === 'REJECTED' || v.isarchived) && (!archiveSearch || (v.title?.toLowerCase().includes(archiveSearch.toLowerCase()) || v.platform?.toLowerCase().includes(archiveSearch.toLowerCase())))).length === 0 ? (
              <div className="bg-[#222] rounded-xl border border-[#333] shadow-sm p-12 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-50 mb-2">Chưa có bài viết lưu trữ phù hợp</h3>
                <p className="text-sm text-slate-400 max-w-sm mx-auto">Danh sách các bài viết bị từ chối, xóa, hoặc để làm sau sẽ nằm ở đây.</p>
              </div>
            ) : (
              <div className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden auto-cols-auto">
                <div className="grid grid-cols-12 gap-4 p-4 border-b border-[#333] bg-[#1c1c1c] text-xs font-bold text-slate-400 uppercase tracking-wider items-center hidden sm:grid">
                ...
                <div className="divide-y divide-gray-100">
                  {videos.filter(v => (v.status === 'REJECTED' || v.isarchived) && (!archiveSearch || (v.title?.toLowerCase().includes(archiveSearch.toLowerCase()) || v.platform?.toLowerCase().includes(archiveSearch.toLowerCase())))).map(video => (
                    <div 
                      key={video.id} 
                      onClick={() => { setEditingTask(video); setIsTaskModalOpen(true); }}
                      className="grid grid-cols-1 sm:grid-cols-12 gap-4 p-4 items-center hover:bg-[#1c1c1c] transition-colors group cursor-pointer"
                    >
                      <div className="col-span-1 sm:col-span-5 flex flex-col justify-center min-w-0">
                        <h3 className="font-bold text-slate-50 text-sm mb-0.5 truncate group-hover:text-indigo-600 transition-colors" title={video.title}>{video.title}</h3>
                        <p className="text-xs text-slate-400 flex items-center gap-1 truncate"><LayoutTemplate className="w-3 h-3 shrink-0"/> <span className="truncate">{video.project} • {video.assignee}</span></p>
                      </div>
                      
                      <div className="sm:hidden flex flex-wrap gap-2 text-[11px] font-medium border-t border-[#333] pt-2 mt-1 w-full">
                        ...
                        <div className="flex gap-1 items-center px-2 py-1 bg-[#2a2a2a] text-gray-600 rounded-md"><Flag className="w-3 h-3"/> {STATUS_MAP[video.status]?.name || 'Từ chối / Để làm sau'}</div>
                        <div className="flex gap-1 items-center px-2 py-1 bg-[#1c1c1c] text-slate-400 rounded-md"><CalendarIcon className="w-3 h-3"/> {video.dueDate ? format(new Date(video.dueDate), 'dd/MM/yyyy') : '-'}</div>
                      </div>

                      <div className="hidden sm:flex col-span-2 items-center justify-center">
                        <span className={`px-2.5 py-1 text-[11px] font-bold rounded-lg border flex items-center gap-1 ${video.platform && video.platform !== 'Khác' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-[#2a2a2a] text-gray-600 border-[#333]'}`}>
                      ...
                      <div className="hidden sm:flex col-span-2 items-center justify-center text-xs font-semibold text-slate-400">
        ...
        {showVideoModal && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Trạng thái</span><div className="mt-1"><span className={`px-2 py-1 rounded-md text-xs font-semibold ${STATUS_MAP[showVideoModal.status]?.color}`}>{STATUS_MAP[showVideoModal.status]?.name}</span></div></div>
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Người thực hiện</span><div className="mt-1 font-medium text-sm text-slate-50">{showVideoModal.assignee}</div></div>
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Nền tảng</span><div className="mt-1 font-medium text-sm text-slate-50 flex items-center gap-1"><Video className="w-3 h-3 text-gray-400"/> {showVideoModal.platform}</div></div>
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Định dạng</span><div className="mt-1 font-medium text-sm text-slate-50">{showVideoModal.format}</div></div>
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Lịch đăng</span><div className="mt-1 font-medium text-sm text-slate-50">{showVideoModal.dueDate ? format(new Date(showVideoModal.dueDate), 'dd/MM/yyyy') : '-'} {showVideoModal.publishTime}</div></div>
                <div><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Mục tiêu</span><div className="mt-1 font-medium text-sm text-slate-50 flex items-center gap-1"><Target className="w-3 h-3 text-gray-400"/> {showVideoModal.goal || '-'}</div></div>
              </div>
              <div className="pt-4 border-t border-[#333]">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Nội dung chi tiết</span>
                <div className="mt-2 text-sm text-slate-200 whitespace-pre-line leading-relaxed bg-[#1c1c1c] p-4 rounded-xl border border-[#333]">{showVideoModal.contentDetails || 'Chưa có nội dung'}</div>
              </div>
              <div className="pt-4 border-t border-[#333] grid grid-cols-1 md:grid-cols-2 gap-4 pb-8">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Hashtag</span>
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm text-slate-200 italic">{showVideoModal.notes || '-'}</div>
                </div>
              </div>
            </div>
        )}
