...
export const MarketingGantt = () => {
            ...
            if (draggingItem && draggingItem.deltaDays !== 0) {
                ...
                if (item) {
                    ...
                    if (Object.keys(updatePayload).length > 0) {
                        const table = item.type === 'project' ? 'marketing_projects' : 'marketing_tasks';
    ...
    const handleQuickAdd = async (parentId: string, projectId: string, e: React.MouseEvent) => {
        ...
        const { data, error } = await supabase.from('marketing_tasks').insert([newTask]).select();
        ...
        <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
                                                                ...
                                                                <button 
                                                                    onClick={async (e) => {
                                                                        ...
                                                                        if (confirm('Bạn có chắc chắn muốn xóa tác vụ này?')) {
                                                                            await supabase.from('marketing_tasks').delete().eq('id', item.id);
            ...
            {isEditModalOpen && (
                <MarketingTaskModal
                    ...
                    generateNextTaskCode={() => `T${Date.now()}`}
                />
            )}
        </div >
    )
