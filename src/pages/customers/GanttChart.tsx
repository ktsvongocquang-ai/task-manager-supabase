  ...
  return (
    <div className="space-y-6 min-w-max">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-3xl font-bold text-slate-50 uppercase tracking-wider">GANTT CHART</h1>
      </div>

      <div className="flex flex-wrap gap-6 mb-8">
        {/* Box 1 */}
        <div className="border border-gray-300 rounded-sm bg-[#222] w-64 shadow-sm">
          <div className="bg-[#2a2a2a] p-2 text-center font-bold text-sm border-b border-gray-300">NĂM</div>
          <div className="p-2 text-center text-sm border-b border-gray-300">2024</div>
          <div className="bg-[#2a2a2a] p-2 text-center font-bold text-sm border-b border-gray-300">THÁNG</div>
        ...
        {/* Box 2 */}
        <div className="border border-gray-300 rounded-sm bg-[#222] w-80 shadow-sm flex flex-col">
          <div className="bg-[#2a2a2a] p-2 text-center font-bold text-sm border-b border-gray-300">KHÔNG HIỂN THỊ NHỮNG DỰ ÁN<br/>CÓ TRẠNG THÁI SAU</div>
        ...
        {/* Box 3 */}
        <div className="border border-gray-300 rounded-sm bg-[#222] w-80 shadow-sm">
          <div className="bg-[#2a2a2a] p-2 text-center font-bold text-sm border-b border-gray-300 mb-2">HOÀN THÀNH VS CHƯA HT</div>
      ...
      <div className="overflow-x-auto bg-[#222] border border-gray-300 shadow-sm">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              ...
              {days.map(d => (
                <th key={`dow-${d.date}`} className="border border-gray-300 bg-[#222] p-1 text-center text-xs font-normal min-w-[35px]">
                  {d.dow}
                </th>
              ))}
            </tr>
            <tr>
              {days.map(d => (
                <th key={`date-${d.date}`} className="border border-gray-300 bg-[#222] p-1 text-center text-xs font-normal">
                  {d.date}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {projects.map(p => (
              <tr key={p.id} className="h-12">
                <td className="border border-gray-300 p-2 sticky left-0 bg-[#222] z-10">{p.name}</td>
                <td className="border border-gray-300 p-2 text-center sticky left-[250px] bg-[#222] z-10">{p.client}</td>
                <td className="border border-gray-300 p-2 text-center sticky left-[430px] bg-[#222] z-10">{formatDate(p.start)}</td>
                <td className="border border-gray-300 p-2 text-center sticky left-[550px] bg-[#222] z-10">{formatDate(p.end)}</td>
                <td className="border border-gray-300 p-2 text-center sticky left-[670px] bg-[#222] z-10">{p.progress}%</td>
                <td className="border border-gray-300 p-2 text-center sticky left-[770px] bg-[#222] z-10 whitespace-nowrap">
                ...
                {days.map(d => {
                   ...
                   return (
                     <td key={`cell-${p.id}-${d.date}`} className={`border border-gray-300 p-0 relative ${bgClass}`}>
                       {isStart && isActive && (
                         <div className="absolute inset-0 flex items-center pl-1 text-xs text-slate-100 z-0 whitespace-nowrap overflow-visible">
                           {p.progress}%
                         </div>
                       )}
                     </td>
