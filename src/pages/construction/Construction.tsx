...
import { Plus, Bell, Camera, Upload, Download, Folder, FileText, Users, Settings, ChevronLeft, Calendar, DollarSign, FileSpreadsheet, CheckCircle2, Copy, X, MessageSquare, Clock, FileSearch, LayoutTemplate, MousePointer2, Image as ImageIcon, Circle, Square, Type, Lightbulb, Box, Palette, ChevronRight } from 'lucide-react';
...
export const Construction = () => {
  ...
  const renderMoodboard = () => (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-4 flex items-center justify-between bg-white sticky top-0 z-10 border-b border-slate-200">
        ...
        <button className="bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm text-white px-6 py-2 rounded-lg text-sm font-bold">
          Lưu Bảng
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-slate-50/50 p-4 relative">
        ...
        {/* Canvas Area */}
        <div className="flex-1 bg-white rounded-2xl border border-slate-300 shadow-inner relative overflow-hidden h-[600px]" style={{ backgroundImage: 'radial-gradient(#cbd5e1 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
           {/* Mockup of canvas with some images and circles */}
           <div className="absolute top-10 left-[200px] w-32 h-32 rounded-full border-4 border-indigo-100 overflow-hidden shadow-xl transform rotate-[-5deg] hover:scale-105 transition-transform cursor-move">
             <img src="https://picsum.photos/seed/wood/200/200" className="w-full h-full object-cover" alt="wood" />
           </div>
           <div className="absolute top-20 left-[350px] w-48 h-48 rounded-full border-4 border-emerald-100 overflow-hidden shadow-xl transform rotate-[10deg] hover:scale-105 transition-transform cursor-move">
             <img src="https://picsum.photos/seed/interior/300/300" className="w-full h-full object-cover" alt="interior" />
           </div>
           <div className="absolute bottom-20 right-[100px] w-64 h-48 bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xl p-3 transform rotate-[-3deg] hover:scale-105 transition-transform cursor-move">
             <img src="https://picsum.photos/seed/furniture/400/300" className="w-full h-full object-cover rounded-lg" alt="furniture" />
             <div className="mt-2 text-center text-sm font-bold text-slate-700 font-serif italic">Góc sofa phòng khách</div>
           </div>
        </div>
      </div>

      {/* Bottom Process Bar */}
      <div className="bg-white border-t border-slate-200 p-3 overflow-x-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      ...
      {currentView === 'HOME' && (
        <div className="mt-8 bg-white border border-slate-200 rounded-2xl p-2 flex flex-wrap sm:flex-nowrap justify-between items-center shadow-sm max-w-3xl mx-auto w-full gap-2">
          ...
          <button 
            ...
            className={`flex-1 flex flex-row sm:flex-col items-center justify-center gap-2 px-4 py-3 rounded-xl transition-all ${activeTab === 'MOODBOARD' ? 'bg-indigo-50 text-indigo-700 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}`}
          >
            <LayoutTemplate className="w-5 h-5" />
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">Moodboard</span>
          </button>
          <button 
