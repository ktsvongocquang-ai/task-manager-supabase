...
const approvalColor = (type: string) => {
  ...
  return colors[type] || 'border-slate-300 bg-[#1c1c1c]';
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={i} className="bg-[#222] rounded-xl border border-[#333] p-4 text-center shadow-sm">
      ...
      <div>
        ...
        <div className="space-y-2">
          {projects.map(p => {
            ...
            return (
              <div key={p.id} onClick={() => onSelectProject(p)} className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all cursor-pointer group">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-slate-100 group-hover:text-indigo-600 transition-colors truncate">{p.name}</h4>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="text-lg font-bold text-slate-100">{p.progress}%</span>
              ...
              <div key={a.id} className={`p-3 rounded-xl border-l-4 ${approvalColor(a.type)} cursor-pointer hover:shadow-sm transition-all`}>
                <div className="flex items-start gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">{a.detail}</p>
      ...
      {/* Cash Flow Forecast */}
      <div className="bg-[#222] rounded-xl border border-[#333] p-5 shadow-sm">
        ...
        <div className="space-y-3">
          {finance.upcoming.map((item, i) => (
            <div key={i} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
              <div>
                <p className="text-xs font-medium text-slate-200">{item.desc}</p>
...
export function EngineerDailyReport({ tasks, project }: { tasks: CTask[]; project: Project }) {
  ...
  if (submitted) {
    return (
      <div className="text-center py-16">
        ...
        <button onClick={() => setSubmitted(false)} className="mt-4 px-4 py-2 border border-[#333] rounded-xl text-sm font-bold text-slate-400 hover:bg-[#1c1c1c]">
          Tạo báo cáo mới
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-bold text-slate-100">Báo cáo hôm nay</h2>
      ...
      <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
        ...
        <p className="text-sm text-slate-100">Hôm nay theo kế hoạch: <strong>{activeTask?.name || 'Không có hạng mục đang thi công'}</strong></p>
        <p className="text-[10px] text-slate-400 mt-1">Thời tiết: Nắng 33°C · GPS: 10.8012, 106.7109</p>
        <div className="flex gap-2 mt-3">
          ...
          <button className="px-4 py-2 border border-[#333] rounded-xl text-xs font-bold text-slate-400 hover:bg-[#1c1c1c]">Sửa hạng mục</button>
        </div>
      </div>

      {/* Workers */}
      <div className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm">
          ...
          {[{ label: 'Thợ chính', key: 'main' as const }, { label: 'Thợ phụ', key: 'helper' as const }].map(w => (
            <div key={w.key} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setWorkers(p => ({ ...p, [w.key]: Math.max(0, p[w.key] - 1) }))} className="w-7 h-7 rounded-lg border border-[#333] flex items-center justify-center text-slate-400 hover:bg-[#1c1c1c] text-sm">-</button>
                <span className="text-lg font-bold text-slate-100 w-6 text-center">{workers[w.key]}</span>
                <button onClick={() => setWorkers(p => ({ ...p, [w.key]: p[w.key] + 1 }))} className="w-7 h-7 rounded-lg border border-[#333] flex items-center justify-center text-slate-400 hover:bg-[#1c1c1c] text-sm">+</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[10px] bg-[#2a2a2a] text-slate-400 px-2 py-0.5 rounded-full font-bold">Lưu trên Cloud Supabase</span>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => fileRef.current?.click()} className="w-14 h-14 rounded-lg border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 hover:border-indigo-400 hover:text-indigo-500 transition-colors bg-[#1c1c1c]">
      ...
      <div className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm">
        ...
        <div className="relative">
          <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Nhập hoặc bấm mic để nói (Speech-to-text)..." rows={3} className={`w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 resize-none transition-all ${isListening ? 'border-rose-300 ring-4 ring-rose-50 bg-rose-50/30' : 'border-[#333] focus:ring-indigo-100'}`} />
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <button onClick={() => { if (isListening) { setIsListening(false); return; } startVoice(); }} className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-xs font-bold transition-all shadow-sm ${isListening ? 'border-rose-300 bg-rose-50 text-rose-600 animate-pulse' : 'border-[#333] text-slate-600 hover:bg-[#1c1c1c]'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center py-6">
        ...
        <p className="text-sm text-slate-400 font-medium">ngày nữa</p>
      ...
      <div className="bg-[#222] rounded-xl border border-[#333] p-5 shadow-sm">
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-[10px] text-slate-400 font-bold">Giá trị hợp đồng</p><p className="text-lg font-bold text-slate-100">{fmt(project.contractValue)}</p></div>
        ...
        <div className="mt-3">
          <div className="w-full h-2 bg-[#2a2a2a] rounded-full overflow-hidden"><div className="h-full bg-emerald-500 rounded-full" style={{ width: `${Math.round(totalPaid / project.contractValue * 100)}%` }} /></div>
      ...
      <div className="grid grid-cols-3 gap-2 min-w-0">
        <div className="bg-[#222] rounded-xl border border-[#333] p-3 text-center shadow-sm">
      ...
      <div>
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Mốc nghiệm thu</h3>
        {(() => {
          ...
          if (!hasRealData && milestones.length === 0) {
            return (
              <div className="bg-[#222] rounded-xl border border-dashed border-[#333] p-6 text-center">
                ...
                return (
                  <div key={item.id}>
                    <div
                      ...
                      className={`bg-[#222] rounded-xl border p-4 shadow-sm flex items-center gap-3 cursor-pointer transition-all ${isDone ? 'border-emerald-200 hover:border-emerald-300' : 'border-amber-200 hover:border-amber-300'}`}
                    >
                    ...
                    {isExpanded && checklist.length > 0 && (
                      <div className="ml-5 mt-1 bg-[#222] border border-[#333] rounded-xl divide-y divide-slate-50 shadow-sm py-1">
                        {checklist.map((c: any) => (
                          <div key={c.id} className="flex items-center gap-3 px-4 py-2.5">
                            ...
                            <span className={`text-xs ${c.completed ? 'line-through text-slate-400' : 'text-slate-200'}`}>{c.label}</span>
                          </div>
                        ))}
      ...
      {phases && phases.length > 0 && (
        <div>
          <button onClick={() => setShowPhases(!showPhases)} className="w-full flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 hover:text-indigo-600 transition-colors">
            Hạng mục thi công
            ...
            <div className="space-y-1 relative">
              ...
              {phases.map(p => (
                <div key={p.id} className="flex items-center gap-3 py-1.5 pl-1 relative">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[10px] z-10 ${p.status === 'done' ? 'bg-emerald-500 text-white' : p.status === 'doing' ? 'bg-blue-500 text-white animate-pulse' : 'bg-[#222] border-2 border-slate-300'}`}>
                    {p.status === 'done' ? '✓' : p.status === 'doing' ? '⚡' : p.order}
                  </div>
                  <div className="flex-1">
                    <p className={`text-[11px] font-medium ${p.status === 'done' ? 'text-slate-400' : p.status === 'doing' ? 'text-blue-600 font-bold' : 'text-slate-400'}`}>{p.name}</p>
      ...
      {dailyLogs && dailyLogs.length > 0 && (
        <div>
          <button onClick={() => setShowDiary(!showDiary)} className="w-full flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 hover:text-indigo-600 transition-colors">
            Nhật ký gần nhất
          ...
          {showDiary && (
            <div className="space-y-2">
              {dailyLogs.slice(0, 5).map(log => (
                <div key={log.id} className="bg-[#222] rounded-xl border border-[#333] p-3 shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-bold text-slate-200">{new Date(log.date).toLocaleDateString('vi-VN')}</p>
                    <div className="flex items-center gap-1">
                      <span className={`text-[9px] px-1.5 py-0.5 rounded border font-bold ${log.weatherAM === 'sunny' ? 'bg-amber-50 text-amber-600 border-amber-200' : log.weatherAM === 'rainy' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-[#1c1c1c] text-slate-400 border-[#333]'}`}>
                        {log.weatherAM === 'sunny' ? '☀️' : log.weatherAM === 'rainy' ? '🌧️' : '⛅'}
                      </span>
                      <span className={`text-[9px] px-1.5 py-0.5 rounded border font-bold ${log.weatherPM === 'sunny' ? 'bg-amber-50 text-amber-600 border-amber-200' : log.weatherPM === 'rainy' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-[#1c1c1c] text-slate-400 border-[#333]'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-bold text-slate-200">Thầu phụ</h3>
      ...
      {subcontractors.map(s => (
        <div key={s.id} className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-bold text-slate-100">{s.name}</p>
              <p className="text-xs text-slate-400 mt-0.5">{s.trade} · {s.phone}</p>
            </div>
          ...
          <div className="flex gap-2 mt-3">
            <button className="px-3 py-1.5 border border-[#333] rounded-lg text-[10px] font-bold text-slate-400 hover:bg-[#1c1c1c]">Gửi báo giá</button>
            <button className="px-3 py-1.5 border border-[#333] rounded-lg text-[10px] font-bold text-slate-400 hover:bg-[#1c1c1c]">Xem lịch sử</button>
          </div>
        </div>
      ))}
    </div>
...
export function AttendanceView({ attendance }: { attendance: AttendanceData | undefined }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#222] rounded-xl border border-[#333] p-5 shadow-sm">
        <h4 className="text-sm font-bold text-slate-200 mb-4">Tuần này</h4>
      ...
      <div className="bg-[#222] rounded-xl border border-[#333] p-5 shadow-sm">
        <h4 className="text-sm font-bold text-slate-200 mb-4">Tháng này</h4>
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-[10px] text-slate-400">Thợ chính: {attendance.thisMonth.main} công</p><p className="text-sm font-bold text-slate-100">{fmt(attendance.thisMonth.main * attendance.dailyRate.main)}</p></div>
          <div><p className="text-[10px] text-slate-400">Thợ phụ: {attendance.thisMonth.helper} công</p><p className="text-sm font-bold text-slate-100">{fmt(attendance.thisMonth.helper * attendance.dailyRate.helper)}</p></div>
        </div>
...
export function ReportsView({ tasks, projectName }: { tasks?: any[]; projectName?: string }) {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-bold text-slate-200">Xu\u1ea5t b\u00e1o c\u00e1o</h3>
      {reports.map((r, i) => (
        <div key={i} className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center border ${r.color}`}><FileText className="w-4 h-4" /></div>
            <div><p className="text-xs font-bold text-slate-100">{r.name}</p><p className="text-[10px] text-slate-400">{r.format}</p></div>
          </div>
          <button onClick={r.action} className="flex items-center gap-1 px-3 py-1.5 border border-[#333] rounded-lg text-[10px] font-bold text-slate-400 hover:bg-[#1c1c1c] hover:border-indigo-300 transition-colors"><Download className="w-3 h-3" /> T\u1ea1o</button>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 justify-end">
          <div className="relative w-64">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm kiếm nhật ký..." className="w-full pl-9 pr-4 py-2 bg-[#222] border border-[#333] rounded-xl text-sm focus:outline-none focus:border-indigo-400" />
      ...
      <div className="bg-[#222] rounded-xl border border-[#333] shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-[#1c1c1c] text-[10px] uppercase text-slate-400 font-bold border-b border-[#333]">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(log => (
              <tr key={log.id} onClick={() => setSelectedLogId(log.id)} className="hover:bg-[#1c1c1c] cursor-pointer transition-colors group">
                <td className="py-3 px-4">
                  <p className="font-bold text-slate-100">{new Date(log.date).toLocaleDateString('vi-VN')}</p>
                  <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                     Sáng: {log.weatherAM === 'sunny' ? '☀️' : log.weatherAM === 'rainy' ? '🌧️' : '⛅'} · Chiều: {log.weatherPM === 'sunny' ? '☀️' : log.weatherPM === 'rainy' ? '🌧️' : '⛅'}
                  </p>
                </td>
                <td className="py-3 px-4">
                  <p className="font-medium text-slate-200 truncate max-w-[180px]" title={log.taskCategory}>{log.taskCategory}</p>
                ...
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-400">{log.reporterName?.[0] || 'KS'}</div>
                    <span className="text-xs font-medium text-slate-200">{log.reporterName || 'Kỹ sư HT'}</span>
                  </div>
                </td>
...
function DailyLogCreateModal({ onClose, onSave }: { onClose: () => void; onSave?: (log: any) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#222] rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
        <div className="px-6 py-4 border-b border-[#333] flex items-center justify-between bg-[#1c1c1c]/50">
          <div>
            <h2 className="text-lg font-bold text-slate-100">Tạo báo cáo ngày</h2>
            <p className="text-xs text-slate-400">Điền thông tin chi tiết các hoạt động tại công trường ngày hôm nay.</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-[#2a2a2a] text-slate-400 hover:bg-slate-200 hover:text-rose-500 transition-colors">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto p-6 bg-[#1c1c1c] text-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <section className="bg-[#222] rounded-xl p-5 border border-[#333] shadow-sm space-y-4">
                <h4 className="text-xs font-bold text-indigo-600 uppercase tracking-wide border-b pb-2">Thông tin chung</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-1">Thời tiết sáng</label>
                    <select value={form.weatherAM} onChange={e => setForm({...form, weatherAM: e.target.value as any})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]">
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-1">Thời tiết chiều</label>
                    <select value={form.weatherPM} onChange={e => setForm({...form, weatherPM: e.target.value as any})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]">
                      <option value="sunny">☀️ Nắng</option><option value="cloudy">⛅ Nhiều mây</option>
                      <option value="rainy">🌧️ Mưa</option><option value="storm">⛈️ Bão</option>
                    </select>
                  </div>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block mb-1">Nhiệt độ trung bình (°C)</label>
                  <input type="number" value={form.temperature} onChange={e => setForm({...form, temperature: +e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]" />
                </div>
              </section>
              ...
              <section className="bg-[#222] rounded-xl p-5 border border-[#333] shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Thợ chính (người)</label><input type="number" value={form.workerMain} onChange={e => setForm({...form, workerMain: +e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]" /></div>
                  <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Thợ phụ (người)</label><input type="number" value={form.workerHelper} onChange={e => setForm({...form, workerHelper: +e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]" /></div>
                </div>
                <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Thiết bị / Máy móc sử dụng</label><input placeholder="VD: Máy trộn, xe cuốc..." value={form.machines} onChange={e => setForm({...form, machines: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c]" /></div>
              </section>
            </div>
            ...
            <div className="space-y-6">
              <section className="bg-[#222] rounded-xl p-5 border border-[#333] shadow-sm space-y-4">
                ...
                <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Công việc đã thi công</label><input placeholder="VD: Xây tường bao tầng lửng..." value={form.taskCategory} onChange={e => setForm({...form, taskCategory: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c] mb-3" /></div>
                <div><label className="text-[10px] font-bold text-slate-400 block mb-1">% Hoàn thành hạng mục</label><input type="number" min="0" max="100" value={form.taskProgress} onChange={e => setForm({...form, taskProgress: +e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c] mb-3" /></div>
                <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Vật tư xuất/nhập/tiêu hao</label><textarea rows={2} placeholder="VD: Xuất 10 bao xi măng, 2 xe cát..." value={form.materials} onChange={e => setForm({...form, materials: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c] resize-none" /></div>
                <div><label className="text-[10px] font-bold text-slate-400 block mb-1">Ghi chú bổ sung</label><textarea rows={2} value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full border rounded-lg px-3 py-2 text-sm bg-[#1c1c1c] resize-none" /></div>
              </section>

              <section className="bg-[#222] rounded-xl p-5 border border-[#333] shadow-sm">
                <div className="flex justify-between items-center mb-4 border-b pb-2">
                  <h4 className="text-xs font-bold text-rose-600 uppercase tracking-wide">Media Thực tế</h4>
                  <span className="text-[10px] bg-[#2a2a2a] text-slate-400 px-2 py-0.5 rounded-full font-bold">{photos.length}/10 File</span>
                </div>
                <div onClick={() => fileRef.current?.click()} className="group border-2 border-dashed border-slate-300 bg-[#1c1c1c] rounded-xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:border-indigo-400 hover:bg-[#222] transition-all min-h-[120px]">
                ...
                {photos.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-[#333]">
                    {photos.map((p, i) => (
                      <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-[#333] shadow-sm group">
                        <img src={p} className="w-full h-full object-cover" />
        ...
        <div className="p-4 border-t border-[#333] bg-[#222] flex justify-end gap-3 rounded-b-2xl">
          <button onClick={onClose} className="px-5 py-2.5 border border-[#333] text-slate-600 rounded-xl text-sm font-bold hover:bg-[#1c1c1c] transition-colors">Hủy thao tác</button>
...
function DailyLogDetailModal({ log, onClose, isManager, onApprove, onReject }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#222] rounded-2xl w-full max-w-5xl h-[95vh] md:h-[85vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-[#333] flex items-center justify-between bg-[#222] relative z-10 shadow-sm">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full border border-[#333] text-slate-400 hover:bg-[#1c1c1c] hover:text-slate-600 transition-colors">✕</button>
        </div>
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden bg-[#1c1c1c]">
          <div className="w-full md:w-[45%] flex flex-col border-b md:border-b-0 md:border-r border-[#333] bg-[#222] max-h-[50%] md:max-h-none">
            <div className="p-6 overflow-y-auto flex-1 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#1c1c1c] p-4 rounded-xl border border-[#333]">
                  ...
                  <p className="text-sm font-medium text-slate-200 flex items-center gap-2">Sáng: {log.weatherAM === 'sunny' ? '☀️ Nắng' : log.weatherAM === 'rainy' ? '🌧️ Mưa' : '⛅ Mây'}</p>
                  <p className="text-sm font-medium text-slate-200 flex items-center gap-2 mt-1">Chiều: {log.weatherPM === 'sunny' ? '☀️ Nắng' : log.weatherPM === 'rainy' ? '🌧️ Mưa' : '⛅ Mây'}</p>
                  {log.temperature && <p className="text-sm font-medium text-slate-200 mt-1">Nhiệt độ: {log.temperature}°C</p>}
                </div>
                <div className="bg-[#1c1c1c] p-4 rounded-xl border border-[#333]">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm text-slate-200 bg-[#1c1c1c] p-4 rounded-xl border border-[#333]">
                  <li className="flex justify-between items-center"><span className="text-slate-400">Thợ chính:</span><span className="font-bold">{log.workerCount.main} người</span></li>
                  <li className="flex justify-between items-center"><span className="text-slate-400">Thợ phụ:</span><span className="font-bold">{log.workerCount.helper} người</span></li>
                  {log.machines && <li className="pt-2 mt-2 border-t border-[#333] flex flex-col"><span className="text-slate-400 text-[10px] mb-1">Máy móc sử dụng:</span><span className="font-medium text-slate-100">{log.machines}</span></li>}
                </ul>
              </div>
              {log.materials && (
                <div>
              ...
              {log.notes && (
                <div>
                  ...
                  <p className="text-sm text-slate-600 bg-[#1c1c1c] p-4 rounded-xl border border-[#333] italic leading-relaxed">{log.notes}</p>
                </div>
              )}
          ...
          <div className="w-full md:w-[55%] flex flex-col bg-[#2a2a2a]/50 relative">
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="bg-[#222] rounded-xl shadow-sm border border-[#333] p-5">
                <h4 className="flex items-center gap-2 text-sm font-bold text-slate-200 mb-4 pb-2 border-b border-[#333]">📸 Hình ảnh thực tế tại công trường</h4>
                <div className="grid grid-cols-3 gap-3">
                  {log.sitePhotos?.length > 0 ? log.sitePhotos.map((p: string, i: number) => (
                    <div key={i} className="aspect-square bg-[#2a2a2a] rounded-lg overflow-hidden border border-[#333] cursor-pointer hover:border-indigo-400 hover:shadow-md transition-all group">
                    ...
                    <div className="col-span-3 text-center py-8 text-sm text-slate-400 bg-[#1c1c1c] rounded-lg border border-dashed border-[#333]">Không có hình ảnh đính kèm</div>
                  )}
                </div>
              </div>
              <div className="bg-[#222] rounded-xl shadow-sm border border-[#333] flex flex-col overflow-hidden max-h-[400px]">
                <div className="px-5 py-3 border-b border-[#333] bg-[#1c1c1c] flex items-center justify-between">
                  <h4 className="text-sm font-bold text-slate-200 flex items-center gap-2">💬 Ý kiến / Nhận xét</h4>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                  {log.comments?.length > 0 ? log.comments.map((c: any) => (
                    <div key={c.id} className="flex gap-3">
                      ...
                      <div className="bg-[#1c1c1c] border border-[#333] rounded-2xl rounded-tl-none p-3 relative flex-1">
                        <p className="text-[10px] font-bold text-slate-400 mb-1 flex items-center justify-between">{c.author} <span className="font-normal text-slate-400">{c.time}</span></p>
                        <p className="text-xs text-slate-200 leading-relaxed">{c.text}</p>
                        {c.images?.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-[#333]">
                            {c.images.map((img: string, i: number) => (
                              <div key={i} className="w-16 h-16 rounded-lg overflow-hidden border border-[#333] cursor-pointer hover:border-indigo-400 hover:shadow-md transition-all">
                ...
                {commentImages.length > 0 && (
                  <div className="px-4 pt-3 pb-1 border-t border-[#333] bg-[#1c1c1c]/80">
                    <div className="flex flex-wrap gap-2">
                      {commentImages.map((img, i) => (
                        <div key={i} className="relative w-14 h-14 rounded-lg overflow-hidden border border-[#333] shadow-sm group">
                ...
                <div className="p-4 border-t border-[#333] bg-[#1c1c1c] flex gap-2 items-center">
                  <input ref={commentFileRef} type="file" accept="image/*" multiple className="hidden" onChange={handleCommentImages} />
                  <button onClick={() => commentFileRef.current?.click()} title="Đính kèm hình ảnh" className="w-9 h-9 flex items-center justify-center rounded-xl border border-[#333] bg-[#222] text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-300 transition-colors shrink-0">
                    📎
                  </button>
                  <input value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendComment(); } }} placeholder="Nhập ý kiến hoặc phản hồi..." className="flex-1 px-3 py-2 border border-[#333] rounded-xl text-xs focus:outline-none focus:border-indigo-400 bg-[#222]" />
        ...
        {isManager && log.status === 'pending' && (
          <div className="p-4 border-t border-[#333] bg-[#222] flex justify-end items-center gap-3 rounded-b-2xl shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)] relative z-20">
            <p className="text-xs text-slate-400 font-medium mr-auto">Báo cáo đang chờ phê duyệt từ quản lý.</p>
        ...
        {log.status === 'approved' && (
          <div className="p-4 border-t border-emerald-100 bg-emerald-50 flex items-center gap-3 rounded-b-2xl">
            ...
            <button onClick={onClose} className="px-5 py-2.5 border border-[#333] text-slate-600 rounded-xl text-sm font-bold hover:bg-[#1c1c1c] transition-colors">Đóng</button>
          </div>
        )}
        {log.status === 'rejected' && (
...
export function ProjectOverview({ project, subcontractors, milestones }: {
  ...
  const totalPaid = milestones.filter(m => m.paymentStatus === 'paid').reduce((s, m) => s + m.paymentAmount, 0);
  const cards = [
    { label: 'Giá trị HĐ đang quản lý', value: fmt(project.contractValue), color: 'text-slate-100', bg: 'bg-[#222]' },
    ...
    { label: 'Nhà thầu & NCC', value: String(subcontractors.length), color: 'text-slate-200', bg: 'bg-[#222]' },
    { label: 'Hồ sơ & Tài liệu', value: String(project.totalDocuments), color: 'text-slate-200', bg: 'bg-[#222]' },
  ...
  return (
    <div className="space-y-3">
      {cards.map((c, i) => (
        <div key={i} className={`rounded-xl border border-[#333] p-4 shadow-sm ${c.bg}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-200">💰 Lịch sử thanh toán</h3>
      ...
      <div className="relative">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tìm kiếm..." className="w-full pl-9 pr-4 py-2.5 bg-[#222] border border-[#333] rounded-xl text-sm focus:outline-none focus:border-indigo-400" />
        <span className="absolute left-3 top-2.5 text-slate-400">🔍</span>
      </div>
      <div className="space-y-2">
        {filtered.map(p => (
          <div key={p.id} className="bg-[#222] rounded-xl border border-[#333] p-3 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-slate-200">{new Date(p.date).toLocaleDateString('vi-VN')}</p>
...
export function ContractorProgressChart({ subcontractors }: { subcontractors: import('./types').Subcontractor[] }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2">📊 Tiến độ nhà thầu</h3>
      <div className="bg-[#222] rounded-xl border border-[#333] p-4 shadow-sm space-y-4">
        {withData.map(s => {
          const paymentPct = s.contractAmount ? Math.round((s.paidAmount || 0) / s.contractAmount * 100) : 0;
          return (
            <div key={s.id}>
              <div className="flex items-center justify-between mb-1">
                <p className="text-[11px] font-bold text-slate-200 truncate">{s.name}</p>
                <p className="text-[9px] text-slate-400">{fmt(s.contractAmount || 0)}</p>
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-3 bg-[#2a2a2a] rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${s.progressPercent || 0}%` }} /></div>
                  <span className="text-[9px] font-bold text-blue-600 w-8 text-right">{s.progressPercent || 0}%</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-3 bg-[#2a2a2a] rounded-full overflow-hidden"><div className="h-full bg-emerald-400 rounded-full transition-all" style={{ width: `${paymentPct}%` }} /></div>
