...
// 1. Remove getTimelineRange and replace with 1 month logic
...
const oldGridEnd = c.indexOf('{/* Mobile View Placeholder */}');
                                            ...
                                            <div className="relative" style={{ width: \`\${totalDays * cellWidth}px\` }}>
                                                ...
                                                {item.isPhase && (
                                                    <div
                                                        ...
                                                        style={{
                                                            left: \`\${(ganttItems.find(p => p.id === item.task?.project_id)?.startIndex ?? 0) * cellWidth}px\`,
                                                            width: \`\${(ganttItems.find(p => p.id === item.task?.project_id)?.duration ?? 0) * cellWidth}px\`,
                                                ...
                                                {item.duration > 0 && (
                                                    <div
                                                        className={\`absolute top-1 bottom-1 rounded-sm shadow-sm flex items-center px-2 cursor-pointer transition-all hover:brightness-95 hover:shadow-md border \${item.task?.status?.includes('Hoàn thành') ? 'bg-emerald-500 border-emerald-600' : item.type === 'project' ? 'bg-[#4a80bc] border-[#3a689b]' : 'bg-[#5da0ea] border-[#4b82c3]'}\`}
                                                        style={{ left: \`\${item.startIndex * cellWidth}px\`, width: \`\${item.duration * cellWidth}px\` }}
                                                        onClick={(e) => { e.stopPropagation(); setTaskPanelMode(item.type === 'project' ? 'project' : 'phase'); setSelectedItem(item); }}
                                                    />
                                                )}
...
// Add isToday and isWeekend and getDayName
const isTodayHelpers = `
...
c = c.replace('const DAY_NAMES', isTodayHelpers + '\nconst DAY_NAMES');

fs.writeFileSync(path, c);
console.log('Reverted to 1 month and unified layout');
