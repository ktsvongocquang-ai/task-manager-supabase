...
import { 
  ...
  BarChart2, X, FileText, Pin, CheckSquare, Square, Archive, GripVertical
} from 'lucide-react';
...
export default function MyTasks() {
  ...
  const [categoryNewTaskTitle, setCategoryNewTaskTitle] = useState('');
  ...
  const handleAddTaskToCategory = (categoryId: string) => {
    if (!categoryNewTaskTitle.trim()) return;
    
    const newTask: Task = {
      id: Date.now().toString(),
      title: categoryNewTaskTitle.trim(),
      status: 'todo',
      dueDate: todayStr,
      category: categoryId,
      priority: 'none'
  ...
  const renderFocusTaskItem = (task: Task) => {
    const isEditing = editingTaskId === task.id;

    return (
      <div 
        key={task.id}
        className={`group flex items-start gap-3 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 px-2 -mx-2 rounded-lg transition-colors ${isEditing ? 'bg-gray-50' : ''}`}
      >
        <div className="mt-1 cursor-grab text-gray-400 opacity-0 group-hover:opacity-100">
          ...
          {isEditing ? (
            <div className="flex flex-col gap-1.5">
              <input
                ...
                className="bg-blue-50 text-blue-900 px-2 py-1 rounded outline-none font-medium w-full"
              />
              <textarea
                value={task.description || ''}
                onChange={(e) => updateTask(task.id, { description: e.target.value })}
                placeholder="Thêm chi tiết công trình này là..."
                className="bg-transparent text-gray-500 outline-none text-sm w-full px-2 resize-none min-h-[80px]"
                rows={4}
              />
              <div className="flex flex-wrap gap-2 mt-2 px-2">
                <button 
                  onMouseDown={(e) => { e.preventDefault(); updateTask(task.id, { dueDate: todayStr }); }}
                  className={`px-3 py-1 rounded-full border ${task.dueDate === todayStr ? 'border-blue-500 text-blue-600 bg-blue-50' : 'border-gray-200 text-gray-600 hover:bg-gray-100'} text-xs font-medium transition-colors`}
                >
                  Hôm nay
                </button>
                <button 
                  onMouseDown={(e) => { e.preventDefault(); updateTask(task.id, { dueDate: tomorrowStr }); }}
                  className={`px-3 py-1 rounded-full border ${task.dueDate === tomorrowStr ? 'border-blue-500 text-blue-600 bg-blue-50' : 'border-gray-200 text-gray-600 hover:bg-gray-100'} text-xs font-medium transition-colors`}
                >
                  Ngày mai
                </button>
                <button className="p-1 rounded-full border border-gray-200 text-gray-600 hover:bg-gray-100 transition-colors">
                  <CalendarIcon className="w-3.5 h-3.5" />
                </button>
              </div>
            </div>
          ) : (
            <div onClick={() => setEditingTaskId(task.id)} className="cursor-text px-2 py-1 hover:bg-gray-100/50 rounded">
              <div className={`font-medium truncate ${task.status === 'done' ? 'line-through text-gray-400' : 'text-gray-800'}`}>
                {task.title}
              </div>
              {task.description && (
                <div className="text-sm text-gray-500 mt-0.5 line-clamp-2 whitespace-pre-wrap">
                  {task.description}
                </div>
              )}
              {task.dueDate && (
                <div className="flex items-center gap-1 mt-1.5">
                  <span className="px-2 py-0.5 rounded border border-gray-200 text-[10px] text-gray-500 flex items-center gap-1">
                    <CalendarIcon className="w-3 h-3" />
                    {task.dueDate === todayStr ? 'Hôm nay' : task.dueDate === tomorrowStr ? 'Ngày mai' : new Date(task.dueDate).toLocaleDateString('vi-VN')}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => handleDeleteTask(task.id)} className="text-gray-400 hover:text-red-500 p-1.5 rounded-md hover:bg-gray-100">
            <Trash2 className="w-4 h-4" />
          </button>
  ...
  const renderNotesView = () => {
    ...
    const NoteCard = ({ note }: { note: Note, key?: any }) => {
    ...
    <div className="h-full flex flex-col bg-[#F8FAFC]">
      ...
      <div className="max-w-5xl mx-auto w-full px-4 sm:px-8 py-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
          {/* View Toggle */}
          <div className="flex items-center p-1 bg-gray-200/50 rounded-xl w-full overflow-x-auto hide-scrollbar">
            <div className="flex w-full min-w-max sm:min-w-0">
              <button 
                onClick={() => setViewMode('focus')}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${viewMode === 'focus' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <Star className="w-4 h-4 flex-shrink-0" /> <span className="hidden sm:inline">Focus Hôm nay</span><span className="sm:hidden">Focus</span>
              </button>
              <button 
                onClick={() => setViewMode('kanban')}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${viewMode === 'kanban' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <LayoutGrid className="w-4 h-4 flex-shrink-0" /> <span className="hidden sm:inline">Bảng Kanban</span><span className="sm:hidden">Kanban</span>
              </button>
              <button 
                onClick={() => setViewMode('calendar')}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${viewMode === 'calendar' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <CalendarIcon className="w-4 h-4 flex-shrink-0" /> <span>Lịch</span>
              </button>
              <button 
                onClick={() => setViewMode('dashboard')}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${viewMode === 'dashboard' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <BarChart2 className="w-4 h-4 flex-shrink-0" /> <span className="hidden sm:inline">Thống kê</span><span className="sm:hidden">Chart</span>
              </button>
              <button 
                onClick={() => setViewMode('notes')}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all ${viewMode === 'notes' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                <FileText className="w-4 h-4 flex-shrink-0" /> <span className="hidden sm:inline">Ghi chú</span><span className="sm:hidden">Note</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {viewMode === 'notes' ? (
          ...
          <div className="flex gap-6 overflow-x-auto pb-8 min-h-[500px] hide-scrollbar">
            {(Object.values(categories) as CategoryItem[]).map(category => {
              const categoryTasks = todayTasks.filter(t => t.category === category.id);
              const isAdding = addingTaskToCategory === category.id;
              
              // Extract base color for styling
              const baseColorMatch = category.color.match(/bg-([a-z]+)-/);
              const baseColor = baseColorMatch ? baseColorMatch[1] : 'gray';
              
              return (
                <div key={category.id} className="w-[320px] flex-shrink-0 flex flex-col">
                  {/* Category Header */}
                  <div className={`rounded-t-2xl p-4 flex items-center gap-2 border-t border-l border-r border-${baseColor}-100 bg-${baseColor}-50/50`}>
                    <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shadow-sm">
                      {category.icon}
                    </div>
                    <h3 className={`font-bold uppercase tracking-wider text-${baseColor}-700 text-sm`}>
                      {category.label}
                    </h3>
                  </div>
                  
                  {/* Category Content */}
                  <div className={`flex-1 bg-white border-l border-r border-b border-${baseColor}-100 rounded-b-2xl p-3 flex flex-col gap-3`}>
                    
                    {/* Add Task Input */}
                    <div 
                      className={`border border-dashed rounded-xl p-3 transition-colors ${isAdding ? `border-${baseColor}-400 bg-${baseColor}-50/30` : `border-gray-200 hover:border-${baseColor}-300 cursor-pointer`}`}
                      onClick={() => !isAdding && setAddingTaskToCategory(category.id)}
                    >
                      {isAdding ? (
                        <div className="flex items-center gap-2">
                          <Plus className={`w-4 h-4 text-${baseColor}-500`} />
                          <input
                            autoFocus
                            type="text"
                            value={categoryNewTaskTitle}
                            onChange={(e) => setCategoryNewTaskTitle(e.target.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') handleAddTaskToCategory(category.id);
                              if (e.key === 'Escape') setAddingTaskToCategory(null);
                            }}
                            onBlur={() => {
                              if (categoryNewTaskTitle.trim()) {
                                handleAddTaskToCategory(category.id);
                              } else {
                                setAddingTaskToCategory(null);
                              }
                            }}
                        ...
                        <div className="flex items-center gap-2 text-gray-500">
                          <Plus className="w-4 h-4" />
                          <span className="text-sm">Thêm việc cần làm</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      {categoryTasks.map(task => (
                        <div key={task.id} className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
                          {renderFocusTaskItem(task)}
                        </div>
                      ))}
                    </div>
                  </div>
