...
const PROJECTS_TIMELINE = [
  {
    ...
    design_days: 10,
    rough_construction_days: 10,
    finishing_days: 5,
    interior_days: 8,
    handover_date: '2026-04-03',
    shooting_milestones: [
      { id: 'ms-1', milestone_date: '2026-03-15', content: 'Quay thi công thô', status: 'Chờ quay' },
      { id: 'ms-2', milestone_date: '2026-03-25', content: 'Quay lắp nội thất', status: 'Chờ quay' }
    ],
    daily_logs: [
      { date: '2026-03-15', link: '#' },
      { date: '2026-03-16', link: '#' }
    ]
  },
  {
    id: 'PRJ-002',
    ...
    shooting_milestones: [
      { id: 'ms-3', milestone_date: '2026-03-16', content: 'Chốt layout thiết kế', status: 'Đã quay' }
    ],
    daily_logs: [
      { date: '2026-03-16', link: '#' }
    ]
...
function calculatePhases(project: any) {
  const phases = [];
  let currentDate = new Date(project.actual_start_date);

  if (project.design_days > 0) {
    const endDate = addDays(currentDate, project.design_days - 1);
    phases.push({ name: 'Thiết kế', start: format(currentDate, 'yyyy-MM-dd'), end: format(endDate, 'yyyy-MM-dd'), color: 'bg-blue-500' });
    currentDate = addDays(endDate, 1);
  }
  
  if (project.rough_construction_days > 0) {
    const endDate = addDays(currentDate, project.rough_construction_days - 1);
    phases.push({ name: 'Thi công thô', start: format(currentDate, 'yyyy-MM-dd'), end: format(endDate, 'yyyy-MM-dd'), color: 'bg-orange-500' });
    currentDate = addDays(endDate, 1);
  }

  if (project.finishing_days > 0) {
    const endDate = addDays(currentDate, project.finishing_days - 1);
    phases.push({ name: 'Thi công hoàn thiện', start: format(currentDate, 'yyyy-MM-dd'), end: format(endDate, 'yyyy-MM-dd'), color: 'bg-purple-500' });
    currentDate = addDays(endDate, 1);
  }

  if (project.interior_days > 0) {
    const endDate = addDays(currentDate, project.interior_days - 1);
    phases.push({ name: 'Thi công nội thất', start: format(currentDate, 'yyyy-MM-dd'), end: format(endDate, 'yyyy-MM-dd'), color: 'bg-indigo-500' });
    currentDate = addDays(endDate, 1);
  }

  if (project.handover_date) {
    phases.push({ name: 'Bàn giao', start: project.handover_date, end: project.handover_date, color: 'bg-green-500' });
  }
...
export default function MarketingApp() {
  ...
  const [view, setView] = useState<'WORKFLOW' | 'KANBAN' | 'TIMELINE' | 'KPI' | 'LIST'>('WORKFLOW');
  const [isRequestModalOpen, setIsRequestModalOpen] = useState(false);
  const [isProgressModalOpen, setIsProgressModalOpen] = useState(false);
  const [selectedProject, setSelectedProject] = useState<any>(null);
  const [projectsTimeline, setProjectsTimeline] = useState(PROJECTS_TIMELINE);
  const [currentMonth, setCurrentMonth] = useState(new Date(2026, 2, 1)); // March 2026 to match mock data
        ...
        <div className="flex-1 overflow-x-auto overflow-y-auto p-6 bg-white">
          <div className="min-w-[1000px] max-w-7xl mx-auto">
            ...
            <div className="flex border-b border-gray-200 pb-2 mb-4 sticky top-0 bg-white z-30">
              <div className="w-64 flex-shrink-0 font-bold text-gray-700 text-sm">Dự án ({format(currentMonth, 'MM/yyyy')})</div>
              ...
              {projectsTimeline.map(project => (
                <div key={project.id} className="flex items-center group">
                  <div className="w-64 flex-shrink-0 pr-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className="text-[10px] text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded transition-colors"
                      >
                        Cập nhật
                      </button>
                    </div>
                    ...
                    <div className="mt-2">
                      <a href="#" className="text-[11px] text-indigo-600 hover:text-indigo-800 hover:underline flex items-center gap-1">
                      ...
                      {project.daily_logs && project.daily_logs.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {project.daily_logs.map((log: any, idx: number) => (
                            <a key={idx} href={log.link} target="_blank" rel="noreferrer" className="text-[9px] bg-blue-50 text-blue-600 hover:bg-blue-100 px-1.5 py-0.5 rounded border border-blue-200 transition-colors">
                  ...
                  <div className="flex-1 relative h-16 bg-gray-50 rounded-xl border border-gray-100 group-hover:bg-gray-100/50 transition-colors">
                    ...
                    {/* Render phases */}
                    {calculatePhases(project).map(phase => {
                       const startDay = parseInt(phase.start.split('-')[2]);
                       const endDay = parseInt(phase.end.split('-')[2]);
                       
                       // Only show phases that overlap with current month
                       const phaseStartMonth = parseInt(phase.start.split('-')[1]);
                       const phaseEndMonth = parseInt(phase.end.split('-')[1]);
                       const currentMonthNum = currentMonth.getMonth() + 1;
                       
                       if (phaseStartMonth !== currentMonthNum && phaseEndMonth !== currentMonthNum) return null;
                       
                       let drawStartDay = startDay;
                       let drawEndDay = endDay;
                       
                       if (phaseStartMonth < currentMonthNum) drawStartDay = 1;
                       if (phaseEndMonth > currentMonthNum) drawEndDay = daysInMonth.length;

                       const left = ((drawStartDay - 1) / daysInMonth.length) * 100;
                       const width = ((drawEndDay - drawStartDay + 1) / daysInMonth.length) * 100;
                    ...
                    {project.shooting_milestones?.map((milestone, idx) => {
                       const msMonth = parseInt(milestone.milestone_date.split('-')[1]);
                       if (msMonth !== currentMonth.getMonth() + 1) return null;
                       
                       const day = parseInt(milestone.milestone_date.split('-')[2]);
                       const left = ((day - 1) / daysInMonth.length) * 100;
                       return (
                         <div 
                           key={idx}
                           className="absolute top-0 bottom-0 w-0.5 bg-red-500 z-10 group/milestone cursor-pointer"
                           style={{ left: `${left}%` }}
                         >
                           <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-white border border-red-200 text-red-600 text-[10px] font-bold px-2 py-1 rounded-md shadow-sm whitespace-nowrap z-20 flex items-center gap-1 group-hover:z-30">
                           ...
                           {/* Tooltip */}
                           <div className="absolute top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs rounded py-1 px-2 opacity-0 group-hover/milestone:opacity-100 transition-opacity whitespace-nowrap z-30 pointer-events-none">
                             {milestone.status}
                           </div>
                         </div>
                       )
