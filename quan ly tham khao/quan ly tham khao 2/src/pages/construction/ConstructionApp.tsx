...
import { Plus, Bell, Camera, Upload, Download, Folder, FileText, Users, Settings, ChevronLeft, Calendar, DollarSign, FileSpreadsheet, CheckCircle2, Copy, X, MessageSquare, Clock, FileCheck, FileSearch, LayoutTemplate, MousePointer2, Image as ImageIcon, Circle, Square, Type, Lightbulb, Box, Palette, ChevronRight, Cloud, Sun, CloudRain, Send } from 'lucide-react';
...
export default function ConstructionApp() {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 mb-6">
        <div className="flex bg-[#1C1C28] rounded-xl p-1 border border-gray-800">
          ...
          <button 
            className="flex-1 py-2 rounded-lg flex justify-center items-center text-gray-400 hover:text-white transition-colors"
          ...
          <button 
            className="flex-1 py-2 rounded-lg flex justify-center items-center text-gray-400 hover:text-white transition-colors"
            onClick={() => setCurrentView('DAILY_LOG')}
          >
  ...
  const copyLogToZalo = (log: any) => {
    const text = `NHẬT KÝ THI CÔNG - ${log.date}
Công trình: ${selectedProject?.name}

1. Thời tiết: ${log.weather}
2. Nhân sự: ${log.mainWorkers} thợ chính, ${log.subWorkers} thợ phụ
3. Công việc hôm nay:
${log.tasksCompleted}
...
${log.issues ? `5. Vấn đề phát sinh:\n${log.issues}` : ''}
`;
    navigator.clipboard.writeText(text);
    alert('Đã copy nội dung nhật ký. Bạn có thể dán vào Zalo!');
  };
    ...
    <div className="flex flex-col h-full bg-[#0A0A0F]">
      <div className="p-4 flex items-center justify-between bg-[#1C1C28] sticky top-0 z-10 border-b border-gray-800">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-white truncate">Nhật Ký Thi Công</h1>
        </div>
        <button 
      ...
      <div className="px-4 py-3 bg-[#1C1C28] border-b border-gray-800">
        <div className="flex bg-[#0A0A0F] rounded-xl p-1 border border-gray-800">
          <button className="flex-1 py-2 rounded-lg flex justify-center items-center text-gray-400 hover:text-white transition-colors" onClick={() => setCurrentView('PROJECT_DETAIL')}>
            <Folder className="w-5 h-5" />
          </button>
          <button className="flex-1 py-2 rounded-lg flex justify-center items-center text-gray-400 hover:text-white transition-colors" onClick={() => setCurrentView('PLANNING')}>
            <Calendar className="w-5 h-5" />
          </button>
          <button className="flex-1 py-2 rounded-lg flex justify-center items-center text-gray-400 hover:text-white transition-colors" onClick={() => setCurrentView('MOODBOARD')}>
            <LayoutTemplate className="w-5 h-5" />
          </button>
          <button className="flex-1 py-2 rounded-lg flex justify-center items-center bg-indigo-600 text-white transition-colors">
            <FileCheck className="w-5 h-5" />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        {isAddingLog && (
          <div className="bg-[#1C1C28] rounded-2xl p-4 border border-indigo-500 animate-in fade-in slide-in-from-top-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-white">Báo Cáo Ngày {new Date().toLocaleDateString('vi-VN')}</h3>
              <button onClick={() => setIsAddingLog(false)} className="text-gray-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Thời tiết</label>
                <div className="flex gap-2">
                  <button onClick={() => setNewLogData({...newLogData, weather: 'Nắng'})} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 text-sm ${newLogData.weather === 'Nắng' ? 'bg-orange-500/20 text-orange-400 border border-orange-500/50' : 'bg-gray-800 text-gray-400 border border-transparent'}`}><Sun className="w-4 h-4" /> Nắng</button>
                  <button onClick={() => setNewLogData({...newLogData, weather: 'Mưa'})} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 text-sm ${newLogData.weather === 'Mưa' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/50' : 'bg-gray-800 text-gray-400 border border-transparent'}`}><CloudRain className="w-4 h-4" /> Mưa</button>
                  <button onClick={() => setNewLogData({...newLogData, weather: 'Râm mát'})} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 text-sm ${newLogData.weather === 'Râm mát' ? 'bg-gray-500/20 text-gray-300 border border-gray-500/50' : 'bg-gray-800 text-gray-400 border border-transparent'}`}><Cloud className="w-4 h-4" /> Râm mát</button>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Thợ chính</label>
                  <input type="number" value={newLogData.mainWorkers} onChange={e => setNewLogData({...newLogData, mainWorkers: parseInt(e.target.value) || 0})} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Thợ phụ</label>
                  <input type="number" value={newLogData.subWorkers} onChange={e => setNewLogData({...newLogData, subWorkers: parseInt(e.target.value) || 0})} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Công việc hôm nay</label>
                <textarea value={newLogData.tasksCompleted} onChange={e => setNewLogData({...newLogData, tasksCompleted: e.target.value})} rows={3} placeholder="- Xây tường bao lầu 1&#10;- Đi ống điện âm tường" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500 resize-none"></textarea>
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Kế hoạch ngày mai</label>
                <textarea value={newLogData.tasksTomorrow} onChange={e => setNewLogData({...newLogData, tasksTomorrow: e.target.value})} rows={2} placeholder="- Đổ bê tông cột&#10;- Nhập vật tư xi măng" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-indigo-500 resize-none"></textarea>
              </div>

              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Hình ảnh hiện trường</label>
                <div className="border-2 border-dashed border-gray-700 rounded-lg p-4 flex flex-col items-center justify-center text-center bg-gray-800/50">
                  <Camera className="w-6 h-6 text-gray-500 mb-2" />
                  <span className="text-xs text-gray-400">Chụp ảnh hoặc tải lên</span>
                  <span className="text-[10px] text-indigo-400 mt-1 flex items-center gap-1"><Cloud className="w-3 h-3" /> Tự động lưu Google Drive</span>
                </div>
              </div>

              <button onClick={handleSaveLog} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700">
                Lưu Nhật Ký
              </button>
            </div>
          </div>
        ...
        {dailyLogs.length === 0 && !isAddingLog ? (
          <div className="text-center py-10">
            <FileCheck className="w-12 h-12 text-gray-700 mx-auto mb-3" />
            <p className="text-gray-400">Chưa có nhật ký thi công nào.</p>
            <button onClick={() => setIsAddingLog(true)} className="text-indigo-400 text-sm mt-2 hover:underline">Thêm báo cáo đầu tiên</button>
          </div>
        ) : (
          dailyLogs.map(log => (
            <div key={log.id} className="bg-[#1C1C28] rounded-2xl p-4 border border-gray-800">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h4 className="font-bold text-white">{log.date}</h4>
                  <div className="flex items-center gap-2 text-xs text-gray-400 mt-1">
                    <span className="flex items-center gap-1"><Sun className="w-3 h-3" /> {log.weather}</span>
                    <span>•</span>
                    <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {log.mainWorkers}C - {log.subWorkers}P</span>
                  </div>
                </div>
                <button onClick={() => copyLogToZalo(log)} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/20 flex items-center gap-1 text-xs font-medium">
                  <Send className="w-3 h-3" /> Copy Zalo
                </button>
              </div>
              
              <div className="space-y-3 text-sm">
                <div>
                  <div className="text-gray-500 text-xs mb-1">Đã làm hôm nay:</div>
                  <div className="text-gray-300 whitespace-pre-wrap">{log.tasksCompleted}</div>
                </div>
                <div>
                  <div className="text-gray-500 text-xs mb-1">Kế hoạch ngày mai:</div>
                  <div className="text-gray-300 whitespace-pre-wrap">{log.tasksTomorrow}</div>
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  <div className="w-16 h-16 bg-gray-800 rounded-lg border border-gray-700 flex items-center justify-center overflow-hidden">
                    <img src="https://picsum.photos/seed/build1/100/100" alt="site" className="w-full h-full object-cover opacity-60" />
                  </div>
                  <div className="w-16 h-16 bg-gray-800 rounded-lg border border-gray-700 flex items-center justify-center overflow-hidden">
                    <img src="https://picsum.photos/seed/build2/100/100" alt="site" className="w-full h-full object-cover opacity-60" />
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="h-screen w-full bg-[#0A0A0F] font-sans flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        {currentView === 'DAILY_LOG' && renderDailyLog()}
      </div>
