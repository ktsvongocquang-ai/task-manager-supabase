...
filepath = r"c:\Users\DELL\.gemini\antigravity\scratch\dqh\task-manager-supabase\src\pages\tasks\WeeklyView.tsx"

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace the component logic
new_component_start = """export const WeeklyView = ({ tasks, projects, profiles, onRefresh, onAddTask }: Props) => {
...
start_idx = content.find("export const WeeklyView = ({ tasks, projects, profiles, onRefresh")
end_update_progress = content.find("const updateProgress = async (taskId: string, delta: number) => {")
end_table_header = content.find("{/* ── Table Header ── */}")

new_content = content[:start_idx] + new_component_start + content[end_update_progress:end_table_header] + new_component_render

# need to make sure import ChevronDown is there
...
print("WeeklyView.tsx refactored successfully.")
